...
func DefaultBindings() []Binding {
	return []Binding{
		...
		{Method: "POST", PathPattern: "/api/v1/business-decisions/:id/owner-decisions", ActionType: "agent_approve", Description: "记录 Owner 经营决定"},
		...
		{Method: "POST", PathPattern: "/api/v1/platform-integrations/:id/order-events", ActionType: "sync_inventory", Description: "导入平台订单并变更库存"},
		...
		{Method: "POST", PathPattern: "/api/v1/finance/cash-reconciliations", ActionType: "destructive_data_change", Description: "将实际银行或支付到账与权威平台结算对账"},
		...
		{Method: "DELETE", PathPattern: "/api/v1/auth/extension-devices/:deviceId", ActionType: "permission_change", Description: "撤销浏览器插件设备凭证"},
		...
		{Method: "POST", PathPattern: "/api/v1/listings/:id/publish", ActionType: "auto_publish", Description: "已冻结旧发布入口；要求受控发布 attempt 的图片放行凭证"},
		{Method: "POST", PathPattern: "/api/v1/product-images/release-attestations/:attestation_id/publish-attempts", ActionType: "auto_publish", TargetType: "product_image_release_attestation", TargetIDParam: "attestation_id", Description: "以受控字节和单次图片放行证明执行媒体发布"},
		{Method: "POST", PathPattern: "/api/v1/product-images/publish-attempts/:attempt_id/reconcile", ActionType: "auto_publish", TargetType: "product_image_publish_attempt", TargetIDParam: "attempt_id", Description: "仅查询并核对结果不明的受控媒体发布，不重试发布"},
		...
		{Method: "POST", PathPattern: "/api/v1/listing/products/:product_id/publish/:platform_id", ActionType: "auto_publish", Description: "已冻结旧产品发布入口；不创建任务或外部请求"},
		{Method: "POST", PathPattern: "/api/v1/listing/listing-tasks/:task_id/publish", ActionType: "auto_publish", Description: "已冻结旧任务发布入口；不变更任务状态"},
		...
		{Method: "POST", PathPattern: "/api/v1/aftersales/resolutions", ActionType: "refund_issue", Description: "创建售后处置请求"},
		{Method: "POST", PathPattern: "/api/v1/aftersales/resolutions/:id/decisions", ActionType: "refund_issue", Description: "Owner裁决售后处置"},
		{Method: "POST", PathPattern: "/api/v1/aftersales/resolutions/:id/executions", ActionType: "refund_issue", Description: "登记外部售后执行请求"},
		{Method: "POST", PathPattern: "/api/v1/aftersales/resolutions/:id/receipts", ActionType: "refund_issue", Description: "登记外部售后终局回执"},
