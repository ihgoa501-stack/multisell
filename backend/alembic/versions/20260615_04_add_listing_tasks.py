...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "product_id", sa.BigInteger(), sa.ForeignKey("product.id"), nullable=False
        ),
        sa.Column(
        ...
        sa.Column(
            "product_listing_id",
            sa.BigInteger(),
            sa.ForeignKey("product_listing.id"),
            nullable=True,
        ),
        sa.Column(
            "source_type",
        ...
        sa.Column(
            "status", sa.String(length=50), nullable=False, server_default="blocked"
        ),
    ...
    op.create_index(
        "ix_listing_task_product_platform",
        "listing_task",
