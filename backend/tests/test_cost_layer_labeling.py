...
class TestDecisionCostLayer:
    ...
    async def test_prelisting_decision_returns_estimated_cost_layers(
        self, async_client
    ):
    ...
    @pytest.mark.skip(
        reason="endpoint POST /api/decisions/prelisting/batch not implemented yet"
    )
...
class TestOrderCostLayer:
    ...
    async def test_order_without_snapshot_shows_estimated_shipping_layer(
        self, async_client
    ):
    ...
    async def test_order_with_snapshot_shows_snapshot_shipping_layer(
        self, async_client
    ):
        ...
        assert (
            profit["profit_cost_layer"] == "mixed"
        )  # shipping=snapshot, platform_fee=estimated
