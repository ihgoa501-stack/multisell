  ...
  <div class="max-w-7xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex items-center gap-2">
        <n-button size="small" quaternary @click="handleExport">
          <template #icon>
            <n-icon><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg></n-icon>
          </template>
          导出
        </n-button>
        <n-button type="primary" size="small" :disabled="!result" @click="handleSave">保存决策</n-button>
      </div>
    </div>
          ...
          <div class="px-4 py-2.5">
            <div class="grid grid-cols-[70px_1fr] items-center gap-x-3 gap-y-2">
              ...
              <div class="flex gap-1.5">
                <n-input v-model:value="formData.target_sale_price" size="small" placeholder="售价" class="flex-1" />
                <n-select v-model:value="formData.destination_country" :options="countryOptions" placeholder="目的国" size="small" class="w-[120px]" filterable />
                <n-select v-model:value="platformId" :options="platformOptions" placeholder="平台" size="small" class="w-[110px]" />
                <n-input v-model:value="paymentFeeStr" size="small" placeholder="支付费%" class="w-[70px]" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white border border-[var(--border-light)] rounded-lg">
          ...
          <div class="px-4 py-3">
            ...
            <div v-if="result" class="flex items-center justify-between px-4 py-3 rounded-[6px] border"
              ...
              <div class="flex gap-1.5">
                <n-button size="tiny" quaternary @click="handleCompare">多平台对比</n-button>
                <n-button size="tiny" type="primary" @click="handleSave">保存决策</n-button>
              </div>
            </div>
    ...
    <div v-if="compareResults.length > 0" class="mb-6">
      <div class="flex items-center justify-between mb-3">
        ...
        <n-button size="tiny" quaternary @click="handleExportCompare">导出对比报告</n-button>
      </div>
        ...
        <table class="w-full text-[13px] border-collapse">
          ...
          <tbody>
            ...
            >
              ...
              <td class="py-3 px-3 border-b border-[var(--border-light)]">
                <n-button v-if="item.recommendation === 'approve'" size="tiny" type="primary" ghost @click="handlePublish(item)">上架</n-button>
              </td>
            </tr>
          </tbody>
        </table>
...
async function loadSkuList() {
  try {
    // TODO: 实际应该从 /api/products 带 SKU 信息搜索
    // 先用 demo 数据
    skuList.value = [
