...
export default function DemandCasesPage(){
 ...
 const liveBatch=useMutation({mutationFn:()=>apiClient.post('/v1/demand-cases/research/reviewed-market-permission-batch',{}),onSuccess:()=>{message.success('本轮侦察与独立反证已入账；没有权限候选也是有效结果');void qc.invalidateQueries({queryKey:['demand-cases']})},onError:(e:Error)=>message.error(e.message)});
 return <PageContainer title="候选市场" subtitle="AI 侦察、独立反证和数据现实审计形成的研究案件；这里没有平台默认答案。" loading={query.isLoading} error={query.isError} errorMsg={(query.error as Error|undefined)?.message} onRetry={()=>void query.refetch()} extra={<Button type="primary" loading={liveBatch.isPending} onClick={()=>liveBatch.mutate()}>导入本轮已审阅研究</Button>}>
