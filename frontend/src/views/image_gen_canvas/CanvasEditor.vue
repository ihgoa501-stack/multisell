<template>
  <div class="canvas-editor h-full flex flex-col">
    <!-- 顶栏 -->
    <div class="flex items-center gap-2 px-4 py-2 bg-white border-b border-[var(--border-light)] shrink-0 z-10">
      <n-select
      ...
      />
      <n-button size="tiny" secondary @click="showGenerateModal = true" :disabled="!selectedProductId">
        <template #icon>
          <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3v18M3 12h18"/></svg>
        </template>
        生图
      </n-button>
      <n-button size="tiny" secondary @click="addTextToCanvas" :disabled="!canvasReady">
        <template #icon>
      ...
      <div class="flex-1" />
      <n-button size="tiny" quaternary @click="saveCanvas" :disabled="!selectedProductId">保存</n-button>
    ...
    <!-- 生图弹窗 -->
    <n-modal v-model:show="showGenerateModal" title="AI 生图" preset="card" style="width:540px;max-width:90vw;" :mask-closable="false">
      <div class="space-y-3">
        <n-input v-model:value="genPrompt" type="textarea" :rows="2" placeholder="描述你想要的图片内容，例如：白色耳机在木质桌面上，自然光..." />
        <div class="grid grid-cols-3 gap-2">
          ...
          <n-input-number v-model:value="genCount" :min="1" :max="4" size="small" style="width:100%">
            <template #prefix>数量</template>
          </n-input-number>
        </div>
        <n-button type="primary" block :loading="generating" :disabled="!genPrompt.trim() || !selectedProductId" @click="handleGenerate">
          生成
        </n-button>
      </div>
    </n-modal>
    ...
    <n-modal v-model:show="showEditModal" title="AI 编辑" preset="card" style="width:480px;max-width:90vw;">
      <div class="space-y-3">
        ...
        <n-input v-if="editMode !== 'remove-bg'" v-model:value="editPrompt" type="textarea" :rows="2"
          :placeholder="editMode === 'inpaint' ? '描述要重绘的区域内容...' : '描述扩展区域内容...'"
        />
        <div v-if="editMode === 'outpaint'" class="flex gap-2">
          <n-button v-for="d in directions" :key="d.value" size="tiny"
            :type="editDirection === d.value ? 'primary' : 'default'"
            @click="editDirection = d.value"
          >{{ d.label }}</n-button>
        </div>
        <n-button type="primary" block :loading="generating"
