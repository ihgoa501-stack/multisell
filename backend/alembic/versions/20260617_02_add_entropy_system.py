...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("target_type", sa.String(30), nullable=False, comment="变更目标: personal_rule/skill/threshold/honcho_profile"),
        ...
        sa.Column("source_type", sa.String(30), nullable=False, comment="来源: gds/gds_proxy/human/nudge/auto_extract"),
    ...
    op.create_index("idx_mark_changes_source_time", "rule_mark_change", ["source_type", sa.text("created_at DESC")])
    op.create_index("idx_mark_changes_target", "rule_mark_change", ["target_type", "target_id", sa.text("created_at DESC")])
    ...
    op.create_table(
        ...
        sa.Column("baseline_stddev", sa.Numeric(10, 4), nullable=False, comment="标准差"),
        ...
        sa.Column("consecutive_same_side", sa.Integer(), default=0, comment="连续同侧点数"),
        sa.Column("last_breach_at", sa.DateTime(timezone=True), comment="上次越线时间"),
        sa.Column("baseline_recalc_at", sa.DateTime(timezone=True), nullable=False, comment="基线计算时间"),
        sa.Column("next_recalc_at", sa.DateTime(timezone=True), nullable=False, comment="下次重算时间"),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), comment="创建时间"),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now(), comment="更新时间"),
    )
    op.create_index("idx_spc_user_agent", "spc_control_limit", ["user_id", "agent_id", "decision_point"])
    op.create_unique_constraint("uq_spc_metric", "spc_control_limit", ["user_id", "agent_id", "decision_point", "metric_name"])
