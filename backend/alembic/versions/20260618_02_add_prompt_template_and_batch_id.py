...
def upgrade() -> None:
    ...
    op.create_table('prompt_template',
        ...
        sa.Column('prompt', sa.String(length=2000), nullable=False, comment='正向提示词'),
        ...
        sa.Column('platform_code', sa.String(length=50), nullable=True, comment='关联平台（为空则通用）'),
        sa.Column('is_shared', sa.SmallInteger(), nullable=True, server_default=sa.text('1'), comment='是否团队共享: 0-私有, 1-共享'),
        sa.Column('usage_count', sa.Integer(), nullable=True, server_default=sa.text('0'), comment='使用次数'),
    ...
    op.add_column('product_image_gen',
        sa.Column('batch_id', sa.String(length=36), nullable=True, comment='批量生图的批次标识 (UUID)')
    )
    op.create_index('idx_product_image_gen_batch_id', 'product_image_gen', ['batch_id'])
    op.create_index('idx_prompt_template_platform', 'prompt_template', ['platform_code'])
