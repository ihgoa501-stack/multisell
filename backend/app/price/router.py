...
async def set_price(
    ...
    await OperationLogService.log(
        ...
        module="price",
        action="set_price",
        resource_id=str(data.sku_id),
        content=f"设置价格: sku_id={data.sku_id}, type={data.price_type}, price={data.price}",
...
async def batch_set_price(
    data: PriceBatchCreate,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_permission("price:batch_update")),
):
    ...
    await OperationLogService.log(
        ...
        module="price",
        action="batch_update",
        resource_id=f"sku_ids={data.sku_ids}",
        content=f"批量调价: sku_count={count}, type={data.price_type}, price={data.price}",
        operator=current_user.username,
    )
...
async def get_prices(
    sku_id: int,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_permission("price:view")),
):
