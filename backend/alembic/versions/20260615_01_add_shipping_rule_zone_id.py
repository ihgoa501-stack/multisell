...
def upgrade() -> None:
    op.add_column(
        "shipping_quote_rule",
        sa.Column("zone_id", sa.BigInteger(), nullable=True, comment="物流区域ID，空表示渠道全局规则"),
    )
    ...
    op.create_index("ix_shipping_quote_rule_zone_id", "shipping_quote_rule", ["zone_id"])


def downgrade() -> None:
    op.drop_index("ix_shipping_quote_rule_zone_id", table_name="shipping_quote_rule")
    op.drop_constraint("fk_shipping_quote_rule_zone_id", "shipping_quote_rule", type_="foreignkey")
