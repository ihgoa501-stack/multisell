...
def main():
    ...
    c, d = (
        req("GET", f"/products/{first_product_id}/skus", token=tok)
        if first_product_id
        else (0, {})
    )
    ...
    results.append(
        (
            ...
            f"{n} SKUs for product {first_product_id}",
        )
    )
    ...
    if bid:
        ...
        if c == 200:
            ...
            for it in items:
                ...
                if po:
                    po_map.setdefault(po, []).append(it)
    ...
    if bid:
        ...
        if c == 200:
            r = d["data"]
            loc, exc, cf = (
                r["ledger_rebuilt_count"],
                r["exception_generated_count"],
                r["chain_failure_count"],
            )
            ok = loc >= 1
            results.append(
                (
    ...
    c, d = req(
        ...
        files={"file": ("bills.csv", csv_content, "text/csv")},
        token=tok,
    )
    if c == 200:
        r = d["data"]
    ...
    if bid2:
        ...
        if c == 200:
            r = d["data"]
            classified = (
                r["matched_count"] + r["mismatch_count"] + r["unmatched_count"]
            ) >= 1
            status = (
                "PASS"
                if r["matched_count"] >= 1
                else ("WARN" if classified else "FAIL")
            )
            results.append(
                (
                    "Shipping Reconcile",
            ...
            print(
                f"  {'+' if status == 'PASS' else '?'} matched={r['matched_count']}, mismatch={r['mismatch_count']}, unmatched={r['unmatched_count']}"
            )
        else:
    ...
    c, d = req(
        "POST",
        "/settlements/import",
        files={"file": ("settlements.csv", csv_content, "text/csv")},
        token=tok,
    )
    if c == 200:
        r = d["data"]
    ...
    results.append(
        (
            "Exception List",
            "PASS" if len(exc_list) >= 1 else "WARN",
            f"{len(exc_list)} items",
        )
    )
    ...
    results.append(
        (
            "Frontend Build",
            "PASS" if ok else "FAIL",
            "warnings detected" if has_warnings else "clean",
        )
    )
