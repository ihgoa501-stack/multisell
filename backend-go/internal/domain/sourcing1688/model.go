...
// PrivateCollectionListItem is the Owner collection-box read model. It keeps
// the stored product unchanged while exposing the latest immutable
// observation's parse statuses and relationship summary needed to judge the
// bookmark without opening raw JSON.
type PrivateCollectionListItem struct {
	Sourcing1688Product
