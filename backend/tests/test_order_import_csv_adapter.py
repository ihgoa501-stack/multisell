...
async def _ensure_inventory(db, sku_id: int, quantity: int = 100):
    ...
    if not inv:
        inv = Inventory(
            sku_id=sku_id, quantity=quantity, locked_quantity=0, safety_stock=0
        )
        db.add(inv)
    else:
...
class TestOrderImportCSVAdapter:
    ...
    async def test_import_single_order_multi_sku(self, async_client: AsyncClient):
        async with async_session_factory() as session:
            _, sku1 = await _create_product_and_sku(
                session, sku_code=f"MSKU-{uuid4().hex[:5]}"
            )
    ...
    @pytest.mark.skip(
        reason="current parser groups same order_no into one order; unknown SKUs create order with sku_id=None"
    )
    ...
    @pytest.mark.skip(
        reason="current parser groups same order_no into one order; no in-batch duplicates exist"
    )
    async def test_import_duplicate_platform_order_no_same_batch_no_longer_skipped(
        self, async_client: AsyncClient
    ):
        async with async_session_factory() as session:
            _, sku = await _create_product_and_sku(
                session, sku_code=f"DUP-{uuid4().hex[:5]}"
            )
            await _ensure_inventory(session, sku.id, 100)
            await session.commit()
        csv_text = (
    ...
    async def test_import_cross_batch_duplicate_skipped(
        self, async_client: AsyncClient
    ):
        async with async_session_factory() as session:
            _, sku = await _create_product_and_sku(
                session, sku_code=f"XDUP-{uuid4().hex[:5]}"
            )
            await _ensure_inventory(session, sku.id, 100)
            await session.commit()
        csv_text = (
            f"{_CSV_HEADER}\nEX-XDUP,{sku.code},1,10,邮,+8613600000000,US,addr4,1.0\n"
        )
        _, content = _make_csv(csv_text)
        resp = await async_client.post(
            "/api/order-import/csv",
    ...
    async def test_import_paid_at_sets_order_paid(self, async_client: AsyncClient):
        async with async_session_factory() as session:
            _, sku = await _create_product_and_sku(
                session, sku_code=f"PAID-{uuid4().hex[:5]}"
            )
            await _ensure_inventory(session, sku.id, 100)
            await session.commit()
        csv_text = (
            f"{_CSV_HEADER}\nEX-005,{sku.code},1,10,邮,+8613600000000,US,addr5,3.0\n"
        )
        _, content = _make_csv(csv_text)
        resp = await async_client.post(
            "/api/order-import/csv",
    ...
    async def test_import_tracking_number_saved(self, async_client: AsyncClient):
        async with async_session_factory() as session:
            _, sku = await _create_product_and_sku(
                session, sku_code=f"TRK-{uuid4().hex[:5]}"
            )
            await _ensure_inventory(session, sku.id, 100)
            await session.commit()
        csv_text = (
            f"{_CSV_HEADER}\nEX-006,{sku.code},1,10,邮,+8613600000000,US,addr6,3.0\n"
        )
        _, content = _make_csv(csv_text)
        resp = await async_client.post(
            "/api/order-import/csv",
    ...
    async def test_audit_log_created(self, async_client: AsyncClient):
        async with async_session_factory() as session:
            _, sku = await _create_product_and_sku(
                session, sku_code=f"AUDIT-{uuid4().hex[:5]}"
            )
            await _ensure_inventory(session, sku.id, 100)
            ...
            await session.commit()
        csv_text = (
            f"{_CSV_HEADER}\nEX-007,{sku.code},1,10,邮,+8613600000000,US,addr7,1.0\n"
        )
        _, content = _make_csv(csv_text)
        resp = await async_client.post(
            "/api/order-import/csv",
        ...
        async with async_session_factory() as session:
            ...
            stmt = sa_select(OperationLog).where(
                OperationLog.module == "order_import", OperationLog.action == "import"
            )
...
class TestPermissions:
    ...
    async def test_import_requires_auth(self, async_client: AsyncClient):
        _, content = _make_csv(f"{_CSV_HEADER}\nEX-AUTH,invalid,1,1,d,e,f,g,1\n")
        resp = await async_client.post(
            "/api/order-import/csv",
            params={"source_type": "ozon"},
            files={"file": ("order_import.csv", content, "text/csv")},
    ...
    async def test_import_allowed_with_permission(self, async_client: AsyncClient):
        ...
        async with async_session_factory() as session:
            _, sku = await _create_product_and_sku(
                session, sku_code=f"PERM-{uuid4().hex[:5]}"
            )
            await _ensure_inventory(session, sku.id, 100)
            await session.commit()
        csv_text = (
            f"{_CSV_HEADER}\nEX-PERM,{sku.code},1,10,邮,+8613600000000,US,addr,1.0\n"
        )
        _, content = _make_csv(csv_text)
        resp = await async_client.post(
            "/api/order-import/csv",
