<template>
  <div class="max-w-4xl mx-auto">
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-xl font-bold text-[var(--ant-color-text)]">铺货到平台</h1>
        <p class="text-sm text-[var(--ant-color-text-tertiary)] mt-0.5">选择商品和目标平台，填写平台专属信息后发布</p>
      </div>
    </div>

    <!-- 选择商品 + 平台 -->
    <div class="bg-white border border-[var(--ant-color-border)] rounded-lg p-4 mb-4 space-y-3">
      <div>
        <div class="text-[11px] font-semibold text-[var(--ant-color-text-tertiary)] uppercase tracking-wide mb-2">商品</div>
        <a-select
      ...
      <div>
        <div class="text-[11px] font-semibold text-[var(--ant-color-text-tertiary)] uppercase tracking-wide mb-2">目标平台</div>
        <a-select
    ...
    <!-- 表单 -->
    <div v-if="!selectedProductId || !selectedPlatformId" class="bg-white border border-[var(--ant-color-border)] rounded-lg p-12 text-center">
      <div class="text-4xl mb-3 opacity-30">📋</div>
      <div class="text-base font-medium text-[var(--ant-color-text-secondary)] mb-1">请先选择商品和平台</div>
      <div class="text-sm text-[var(--ant-color-text-tertiary)]">选择后即可编辑发布内容</div>
    </div>

    <div v-else class="space-y-4">
      <!-- 商品信息（只读） -->
      <div class="bg-white border border-[var(--ant-color-border)] rounded-lg p-4">
        <div class="text-[11px] font-semibold text-[var(--ant-color-text-tertiary)] uppercase tracking-wide mb-3">商品信息</div>
        <div class="flex gap-4">
          <div class="w-16 h-16 rounded-[6px] bg-[var(--ant-color-fill-quaternary)] flex items-center justify-center text-2xl shrink-0">{{ currentProduct?.emoji || '📦' }}</div>
          <div class="flex-1 min-w-0">
            <div class="text-[14px] font-medium text-[var(--ant-color-text)]">{{ currentProduct?.name }}</div>
            <div class="text-[12px] text-[var(--ant-color-text-tertiary)] mt-0.5">SKU: {{ currentProduct?.skuCount }} 个 · 成本 ¥{{ currentProduct?.minPrice }}</div>
          </div>
        </div>
      </div>

      <!-- 平台表单 -->
      <div class="bg-white border border-[var(--ant-color-border)] rounded-lg overflow-hidden">
        <div class="px-4 py-3 border-b border-[var(--ant-color-border)] flex items-center justify-between">
          <span class="text-[11px] font-semibold text-[var(--ant-color-text-tertiary)] uppercase tracking-wide">{{ currentPlatform?.name }} 发布信息</span>
          <a-button size="small" :loading="generating" @click="handleGenerateAI">AI 帮我生成</a-button>
        </div>
        ...
        <div class="p-4 space-y-4">
          ...
          <div>
            <div class="flex items-center justify-between mb-1">
              <label class="text-[13px] font-medium text-[var(--ant-color-text)]">标题 <span class="text-red-500">*</span></label>
              <span class="text-[11px] text-[var(--ant-color-text-tertiary)]">{{ form.title.length }}/200</span>
            </div>
            <a-input v-model:value="form.title" placeholder="输入平台商品标题" :maxlength="200" size="small" />
          </div>

          <!-- 公共字段：描述 -->
          <div>
            <div class="flex items-center justify-between mb-1">
              <label class="text-[13px] font-medium text-[var(--ant-color-text)]">描述 <span class="text-red-500">*</span></label>
            </div>
            <a-textarea v-model:value="form.description" placeholder="输入平台商品描述" :rows="4" size="small" />
          </div>
          ...
          <template v-for="field in platformExtraFields" :key="field.key">
            ...
            <div v-if="field.type === 'textarea'" class="grid grid-cols-2 gap-4">
              <div v-for="(_, idx) in field.count" :key="idx">
                <div class="flex items-center justify-between mb-1">
                  <label class="text-[13px] font-medium text-[var(--ant-color-text)]">{{ field.label }} {{ idx + 1 }}</label>
                  <span class="text-[11px] text-[var(--ant-color-text-tertiary)]">{{ (form.extra[field.key] || [])[idx]?.length || 0 }}/500</span>
                </div>
                <a-textarea
            ...
            <div v-else-if="field.type === 'text'">
              <div class="flex items-center justify-between mb-1">
                <label class="text-[13px] font-medium text-[var(--ant-color-text)]">{{ field.label }} <span v-if="field.required" class="text-red-500">*</span></label>
              </div>
              <a-input v-model:value="form.extra[field.key]" :placeholder="field.placeholder || `输入${field.label}`" size="small" />
            </div>
            ...
            <div v-else-if="field.type === 'number'" class="grid grid-cols-2 gap-4">
              <div v-for="(_, idx) in field.count || 1" :key="idx">
                <div class="flex items-center justify-between mb-1">
                  <label class="text-[13px] font-medium text-[var(--ant-color-text)]">{{ field.count && field.count > 1 ? `${field.label} ${idx + 1}` : field.label }} <span v-if="field.required" class="text-red-500">*</span></label>
                </div>
                <a-input-number
            ...
            <div v-else-if="field.type === 'tags'">
              <div class="flex items-center justify-between mb-1">
                <label class="text-[13px] font-medium text-[var(--ant-color-text)]">{{ field.label }}</label>
              </div>
              <div class="flex flex-wrap gap-1 mb-2">
                <a-tag v-for="(tag, ti) in (form.extra[field.key] as string[])" :key="ti" closable @close="(form.extra[field.key] as string[]).splice(ti, 1)" color="processing">{{ tag }}</a-tag>
              </div>
              <div class="flex gap-1">
                <a-input v-model:value="newTag" :placeholder="field.placeholder || '添加'" size="small" @keyup.enter="addTag(field.key)" />
                <a-button size="small" @click="addTag(field.key)">添加</a-button>
              </div>
            </div>
            ...
            <div v-else-if="field.type === 'keyvalue'">
              ...
              <div class="space-y-1.5">
                <div v-for="(kv, ki) in getKVArr(field.key)" :key="ki" class="flex gap-1.5 items-center">
                  <a-input v-model:value="kv.k" placeholder="属性名" size="small" style="width:120px" />
                  <a-input v-model:value="kv.v" placeholder="属性值" size="small" class="flex-1" />
                  <a-button size="small" type="text" @click="getAnyArr(field.key).splice(ki, 1)">×</a-button>
                </div>
                <a-button size="small" @click="getAnyArr(field.key).push({k:'',v:''})">+ 添加</a-button>
              </div>
            </div>
          </template>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <div class="flex items-center justify-between mb-1">
                <label class="text-[13px] font-medium text-[var(--ant-color-text)]">售价 ({{ currentPlatform?.currency || '¥' }}) <span class="text-red-500">*</span></label>
              </div>
              <a-input-number v-model:value="form.salePrice" placeholder="0.00" :min="0" :precision="2" size="small" class="w-full" />
            </div>
            <div>
              <div class="flex items-center justify-between mb-1">
                <label class="text-[13px] font-medium text-[var(--ant-color-text)]">原价 ({{ currentPlatform?.currency || '¥' }})</label>
              </div>
              <a-input-number v-model:value="form.originalPrice" placeholder="0.00" :min="0" :precision="2" size="small" class="w-full" />
            </div>
          </div>
          ...
          <div>
            <div class="flex items-center justify-between mb-1">
              <label class="text-[13px] font-medium text-[var(--ant-color-text)]">平台类目 <span class="text-red-500">*</span></label>
            </div>
            <a-select v-model:value="form.category" :options="categoryOptions" placeholder="选择平台类目" size="small" style="width: 100%" />
          </div>

          <!-- 公共字段：SEO关键词 -->
          <div>
            <div class="flex items-center justify-between mb-1">
              <label class="text-[13px] font-medium text-[var(--ant-color-text)]">SEO 关键词</label>
            </div>
            <div class="flex flex-wrap gap-1 mb-2">
              <a-tag v-for="(kw, ki) in form.keywords" :key="ki" closable @close="form.keywords.splice(ki, 1)" color="processing">{{ kw }}</a-tag>
            </div>
            <div class="flex gap-1">
              <a-input v-model:value="newKeyword" placeholder="添加关键词" size="small" @keyup.enter="addKeyword" />
              <a-button size="small" @click="addKeyword">添加</a-button>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- 操作栏 -->
      <div class="bg-white border border-[var(--ant-color-border)] rounded-lg p-4 flex items-center justify-between">
        <a-button size="small" @click="handleSaveDraft">存草稿</a-button>
        <a-button size="small" type="primary" :disabled="!formValid" @click="handlePublish">发布到 {{ currentPlatform?.name }}</a-button>
      </div>
    </div>
  </div>
</template>
