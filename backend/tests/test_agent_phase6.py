...
class TestA1ProductScout:
    ...
    async def test_scout_with_candidates(self, agent):
        ctx = {
            ...
            "marketplace": "US",
            "candidates": [
                {"name": "Silicone Plate", "price": 19.99, "cost": 4.5,
                 "search_volume": 15000, "trend_growth": 120, "review_count": 200},
                {"name": "Baby Spoon Set", "price": 12.99, "cost": 3.0,
                 "search_volume": 8000, "trend_growth": 80, "review_count": 500},
            ],
        }
    ...
    async def test_market_analysis(self, agent):
        result = await agent.decide("market_analysis", {"category": "Toys", "trend": "growing"})
...
class TestA2ListingOptimizer:
    ...
    async def test_optimize_with_keywords(self, agent):
        ctx = {
            ...
            "marketplace": "US",
            "features": ["Suction base", "Food-grade silicone", "BPA free", "3 compartments"],
            "keywords": [
    ...
    async def test_keyword_research(self, agent):
        result = await agent.decide("keyword_research", {"seed_keywords": ["baby", "plate"]})
...
class TestA4CustomerService:
    ...
    async def test_auto_reply_shipping(self, agent):
        result = await agent.decide("auto_reply", {
    ...
    async def test_intent_classify(self, agent):
        result = await agent.decide("intent_classify", {"message": "How to return this item", "language": "en"})
...
class TestA7Compliance:
    ...
    async def test_compliance_electronics_us(self, agent):
        result = await agent.decide("compliance_check", {
    ...
    async def test_certification_lookup(self, agent):
        result = await agent.decide("certification_lookup", {"certification": "FCC", "country": "US"})
...
class TestG2WarehouseCustoms:
    ...
    async def test_clearance_electronics_us(self, agent):
        result = await agent.decide("customs_clearance", {
    ...
    async def test_warehouse_high_volume(self, agent):
        result = await agent.decide("warehouse_advice", {
            "destination_country": "US",
