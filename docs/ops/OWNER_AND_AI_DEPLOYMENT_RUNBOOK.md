...
当本文件与旧部署文档冲突时，以本文件、[海洋目标](../OCEAN_GOAL.md)、[产品方向](../PRODUCT.md) 和 [当前状态](../CURRENT.md) 为准。治理、安全、审批和审计规则仍以 `docs/governance/` 为最高约束。
...
```text
系统可用 → Owner 确认当前湖泊 → 真实数据与证据决策 → Owner 审批与受控执行 → 订单、履约、结算、最终利润与反馈
```
