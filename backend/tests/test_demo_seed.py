...
class TestDemoCsvParseable:
    ...
    def _read_bytes(rel_path: str) -> bytes:
        demo_dir = os.path.join(
            os.path.dirname(__file__), "..", "..", "docs", "demo-data"
        )
    ...
    def test_shipping_bill_csv_parseable(self):
        rows, errors = ShippingBillService.parse_csv(
            self._read_bytes("shipping_bill_demo.csv")
        )
        assert len(rows) == 5
        assert len(errors) == 0

    def test_shipping_bill_has_matched_and_unmatched(self):
        rows, _ = ShippingBillService.parse_csv(
            self._read_bytes("shipping_bill_demo.csv")
        )
        unmatched = [
            r
            for r in rows
            if r.get("tracking_number") and "UNMATCHED" in r["tracking_number"]
        ]
        matched = [
            r
            for r in rows
            if r.get("tracking_number") and "UNMATCHED" not in r["tracking_number"]
        ]
        matched_by_order = [
            r for r in rows if not r.get("tracking_number") and r.get("order_no")
        ]
    ...
    def test_settlement_csv_parseable(self):
        rows, errors = SettlementService.parse_csv(
            self._read_bytes("platform_settlement_demo.csv")
        )
        assert len(rows) > 0
        assert len(errors) == 0
    ...
    def test_settlement_has_matched_and_unmatched(self):
        ...
        """
        rows, _ = SettlementService.parse_csv(
            self._read_bytes("platform_settlement_demo.csv")
        )
        ...
        unmatched = [
            r
            for r in rows
            if r.get("platform") and "UNMATCHED" in r["platform"].upper()
        ]
        # Matched: 有 platform_order_no 的行
        matched = [
            r
            for r in rows
            if r.get("platform_order_no")
            or (
                r.get("order_no")
                and "UNMATCHED" not in (r.get("order_no") or "").upper()
            )
        ]
...
class TestDemoSeed:
    ...
    async def test_sku_codes_match_csv(self, async_client):
        ...
        with open(
            os.path.join(demo_dir, "order_import_demo.csv"), encoding="utf-8-sig"
        ) as f:
            csv_codes = {
                row["sku_code"].strip()
                for row in csv.DictReader(f)
                if row.get("sku_code", "").strip()
            }
        from scripts.load_demo_data import load_demo_data
...
class TestDemoSeedCreatesData:
    ...
    async def test_users(self, async_client):
        ...
        async with async_session_factory() as session:
            ...
            assert (
                await session.execute(select(User).where(User.username == "demo"))
            ).scalar_one_or_none()
            ...
            prods = (
                (
                    await session.execute(
                        select(Product).where(Product.name.like("Demo %"))
                    )
                )
                .scalars()
                .all()
    ...
    async def test_shipping(self, async_client):
        ...
        async with async_session_factory() as session:
            assert (
                len((await session.execute(select(ShippingProvider))).scalars().all())
                >= 2
            )
            assert (
                len((await session.execute(select(ShippingChannel))).scalars().all())
                >= 3
            )
            assert (
                len((await session.execute(select(ShippingQuoteRule))).scalars().all())
                >= 3
            )
    ...
    async def test_platform_fee_rules(self, async_client):
        ...
        async with async_session_factory() as session:
            assert (
                len((await session.execute(select(PlatformFeeRule))).scalars().all())
                >= 1
            )
