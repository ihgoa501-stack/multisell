...
class TestOrderImportCSVAdapter:
    async def test_import_single_order_single_sku(self, async_client: AsyncClient):
        ...
        csv_text = (
            ...
            f"Ozon,Ozon主店,EX-001,,{sku.code},1,12.5,USD,张三,+8613800000000,US,addr1,5.0,2026-06-01 12:00:00\n"
        )
    ...
    async def test_import_single_order_multi_sku(self, async_client: AsyncClient):
        ...
        csv_text = (
            "platform,store_name,platform_order_no,order_no,sku_code,quantity,unit_price,currency,recipient_name,recipient_phone,country_code,shipping_address,shipping_fee,paid_at\n"
            f"Shopee,店铺A,EX-002,,{sku1.code},2,9.9,CNY,李四,+8613700000000,CN,addr2,2.0,\n"
            f"Shopee,店铺A,EX-002,,{sku2.code},1,15.0,CNY,李四,+8613700000000,CN,addr2,2.0,\n"
        )
        _, content = _make_csv(csv_text)
        resp = await async_client.post("/api/order-imports/csv", files={"file": ("order_import.csv", content, "text/csv")})
        assert resp.status_code == 200
    ...
    async def test_import_missing_sku_item_failed(self, async_client: AsyncClient):
        ...
        csv_text = (
            "platform,store_name,platform_order_no,order_no,sku_code,quantity,unit_price,currency,recipient_name,recipient_phone,country_code,shipping_address,shipping_fee,paid_at\n"
            "Ozon,Ozon主店,EX-003,,NOT_EXIST_SKU,1,10,CNY,邮,+8613600000000,US,addr3,1.0,\n"
        )
        _, content = _make_csv(csv_text)
        resp = await async_client.post("/api/order-imports/csv", files={"file": ("order_import.csv", content, "text/csv")})
        assert resp.status_code == 200
    ...
    async def test_import_duplicate_platform_order_no_skipped(self, async_client: AsyncClient):
        ...
        csv_text = (
            "platform,store_name,platform_order_no,order_no,sku_code,quantity,unit_price,currency,recipient_name,recipient_phone,country_code,shipping_address,shipping_fee,paid_at\n"
            f"Temu,Temu店,EX-004,,{sku.code},1,10,CNY,邮,+8613600000000,US,addr4,1.0,\n"
            f"Temu,Temu店,EX-004,,{sku.code},1,10,CNY,邮,+8613600000000,US,addr4,1.0,\n"
        )
        _, content = _make_csv(csv_text)
        resp = await async_client.post("/api/order-imports/csv", files={"file": ("order_import.csv", content, "text/csv")})
    ...
    async def test_import_paid_at_sets_order_paid(self, async_client: AsyncClient):
        ...
        csv_text = (
            "platform,store_name,platform_order_no,order_no,sku_code,quantity,unit_price,currency,recipient_name,recipient_phone,country_code,shipping_address,shipping_fee,paid_at\n"
            f"Ozon,Ozon主店,EX-005,,{sku.code},1,10,CNY,邮,+8613600000000,US,addr5,3.0,2026-06-02 10:00:00\n"
        )
        _, content = _make_csv(csv_text)
        resp = await async_client.post("/api/order-imports/csv", files={"file": ("order_import.csv", content, "text/csv")})
        assert resp.status_code == 200
    ...
    async def test_import_tracking_number_saved(self, async_client: AsyncClient):
        async with async_session_factory() as session:
            ...
            await session.commit()
        csv_text = (
            "platform,store_name,platform_order_no,order_no,sku_code,quantity,unit_price,currency,recipient_name,recipient_phone,country_code,shipping_address,shipping_fee,paid_at,tracking_number\n"
            f"Ozon,Ozon主店,EX-006,,{sku.code},1,10,CNY,邮,+8613600000000,US,addr6,3.0,,TRACK123\n"
        )
        _, content = _make_csv(csv_text)
        resp = await async_client.post("/api/order-imports/csv", files={"file": ("order_import.csv", content, "text/csv")})
        assert resp.status_code == 200
    ...
    async def test_audit_log_created(self, async_client: AsyncClient):
        ...
        csv_text = (
            "platform,store_name,platform_order_no,order_no,sku_code,quantity,unit_price,currency,recipient_name,recipient_phone,country_code,shipping_address,shipping_fee,paid_at\n"
            f"Ozon,Ozon主店,EX-007,,{sku.code},1,10,CNY,邮,+8613600000000,US,addr7,1.0,\n"
        )
        _, content = _make_csv(csv_text)
        await async_client.post("/api/order-imports/csv", files={"file": ("order_import.csv", content, "text/csv")})
        async with async_session_factory() as session:
...
class TestPermissions:
    ...
    async def test_import_allowed_with_permission(self, async_client: AsyncClient):
        ...
        await grant_permission(user_id, "order_import:import")
        async with async_session_factory() as session:
        ...
        csv_text = (
            "platform,store_name,platform_order_no,order_no,sku_code,quantity,unit_price,currency,recipient_name,recipient_phone,country_code,shipping_address,shipping_fee,paid_at\n"
            f"Ozon,Ozon主店,EX-PERM,,{sku.code},1,10,CNY,邮,+8613600000000,US,addr,1.0,\n"
        )
        _, content = _make_csv(csv_text)
        resp = await async_client.post(
            "/api/order-imports/csv",
