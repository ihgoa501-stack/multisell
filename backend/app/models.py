...
class Product(Base):
    ...
    ai_status = Column(
        String(50), default="pending", comment="AI处理状态: pending/completed/failed"
    )
    # 多平台状态概览
...
class SpecValue(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    spec_name_id = Column(
        BigInteger, ForeignKey("spec_name.id"), nullable=False, comment="规格名称ID"
    )
    product_id = Column(
        BigInteger, ForeignKey("product.id"), nullable=False, comment="商品ID"
    )
...
class Sku(Base):
    ...
    stock = Column(
        Integer, default=0, comment="库存（已弃用，请使用 Inventory.quantity）"
    )
    lock_stock = Column(Integer, default=0, comment="锁定库存")
    warning_stock = Column(Integer, default=0, comment="安全库存预警")
    weight = Column(
        Numeric(10, 2), default=0, comment="重量(kg，历史字段，不表示包装重量)"
    )
...
class Price(Base):
    ...
    price_type = Column(
        String(50),
        nullable=False,
        comment="价格类型: sale_price/market_price/cost_price/vip_price/wholesale_price",
    )
...
class Inventory(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    sku_id = Column(
        BigInteger, ForeignKey("sku.id"), nullable=False, unique=True, comment="SKU ID"
    )
...
class ProductSupplier(Base):
    ...
    supplier_id = Column(
        BigInteger, ForeignKey("supplier.id"), nullable=False, comment="供应商ID"
    )
...
class Sourcing1688Product(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    source_url = Column(
        String(1000), nullable=False, unique=True, comment="1688 商品链接"
    )
    ...
    status = Column(
        String(50), default="collected", comment="状态: collected/imported/rejected"
    )
    product_id = Column(BigInteger, ForeignKey("product.id"), comment="导入后的商品 ID")
    supplier_id = Column(
        BigInteger, ForeignKey("supplier.id"), comment="导入后的供应商 ID"
    )
...
class ShippingChannel(Base):
    ...
    provider_id = Column(
        BigInteger,
        ForeignKey("shipping_provider.id"),
        nullable=False,
        comment="物流供应商ID",
    )
    name = Column(String(200), nullable=False, comment="渠道名称")
    code = Column(String(50), comment="渠道编码")
    volumetric_divisor = Column(
        Integer, nullable=False, default=6000, comment="抛重系数"
    )
    cargo_types = Column(
        JSON, comment='支持货品类型: ["normal","battery","liquid","sensitive"]'
    )
...
class ShippingZone(Base):
    ...
    channel_id = Column(
        BigInteger,
        ForeignKey("shipping_channel.id"),
        nullable=False,
        comment="物流渠道ID",
    )
    country_code = Column(
        String(10), nullable=False, comment="国家代码 ISO 3166-1 alpha-2"
    )
...
class ShippingQuoteRule(Base):
    ...
    zone_id = Column(
        BigInteger,
        ForeignKey("shipping_zone.id"),
        comment="物流区域ID，空表示渠道全局规则",
    )
    rule_type = Column(
        String(50),
        nullable=False,
        comment="规则类型: fixed_plus_per_kg/first_weight_plus_increment/tiered_weight",
    )
    ...
    rounding_increment = Column(
        Numeric(10, 3), default=0.1, comment="计费重向上取整增量(kg)"
    )
    remark = Column(Text, comment="备注")
    status = Column(SmallInteger, default=1, comment="状态: 0-禁用, 1-启用")
    created_at = Column(
...
class Platform(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    name = Column(
        String(100), nullable=False, comment="平台名称（如: Ozon, Shopee, Wildberries）"
    )
    code = Column(
        String(50), nullable=False, unique=True, comment="平台代码（ozon, shopee, wb）"
    )
...
class ProductListing(Base):
    ...
    platform_id = Column(
        BigInteger, ForeignKey("platform.id"), nullable=False, comment="平台ID"
    )
    platform_product_id = Column(String(200), comment="平台上的商品ID")
    platform_sku = Column(String(200), comment="平台上的SKU")
    status = Column(
        String(50), default="draft", comment="发布状态: draft/pending/synced/failed"
    )
...
class ListingTask(Base):
    ...
    product_listing_id = Column(
        BigInteger, ForeignKey("product_listing.id"), comment="发布记录ID"
    )
    source_type = Column(
        String(50), default="decision", nullable=False, comment="来源: decision/manual"
    )
    ...
    status = Column(
        ...
        default="blocked",
        nullable=False,
        comment="ready/blocked/published/failed/cancelled",
    )
    missing_requirements = Column(
        JSON, default=list, nullable=False, comment="阻塞发布的缺失项"
    )
...
class ShippingBillBatch(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    provider_id = Column(
        BigInteger, ForeignKey("shipping_provider.id"), comment="物流供应商ID"
    )
    ...
    status = Column(
        String(30), server_default="imported", comment="imported/reconciled/failed"
    )
    created_by = Column(String(100), comment="创建人")
    created_at = Column(
        DateTime(timezone=True), server_default=func.now(), comment="创建时间"
...
class ShippingBillItem(Base):
    ...
    batch_id = Column(
        BigInteger,
        ForeignKey("shipping_bill_batch.id"),
        nullable=False,
        comment="批次ID",
    )
    ...
    reconciliation_status = Column(
        String(30),
        server_default="unmatched_bill",
        comment="matched/unmatched_bill/missing_snapshot/amount_mismatch/currency_mismatch",
    )
    ...
    matched_order_id = Column(
        BigInteger, ForeignKey("sales_order.id"), comment="匹配订单ID"
    )
    matched_snapshot_id = Column(
        BigInteger, ForeignKey("sales_order_shipping_snapshot.id"), comment="匹配快照ID"
    )
...
class PlatformSettlementBatch(Base):
    ...
    import_status = Column(
        String(30), server_default="imported", comment="imported/partial/complete"
    )
...
class PlatformSettlementItem(Base):
    ...
    batch_id = Column(
        BigInteger,
        ForeignKey("platform_settlement_batch.id"),
        nullable=False,
        comment="批次ID",
    )
    row_number = Column(Integer, nullable=False, comment="原始行号")
    ...
    transaction_type = Column(
        String(50),
        nullable=False,
        comment="交易类型: sale/platform_fee/payment_fee/refund/adjustment/payout/tax/other",
    )
    currency = Column(String(10), server_default="CNY", comment="币种")
    ...
    match_status = Column(
        String(30), server_default="unmatched", comment="matched/unmatched/manual"
    )
    matched_order_id = Column(
        BigInteger, ForeignKey("sales_order.id"), comment="匹配订单ID"
    )
...
class FinanceLedgerEntry(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    order_id = Column(
        BigInteger, ForeignKey("sales_order.id"), nullable=False, comment="订单ID"
    )
    entry_type = Column(
        String(50),
        nullable=False,
        comment="条目类型: revenue/product_cost/shipping_cost/platform_fee/payment_fee/refund/adjustment/other_fee",
    )
    amount = Column(
        Numeric(14, 2), nullable=False, comment="金额（正数为收入，负数为成本/费用）"
    )
    currency = Column(String(10), server_default="CNY", comment="币种")
    cost_layer = Column(
        String(30),
        nullable=False,
        comment="成本层: estimated/snapshot/actual/allocated",
    )
    source_type = Column(
        String(50),
        comment="来源类型: order/shipping_snapshot/shipping_bill_row/settlement_row",
    )
...
class ExceptionItem(Base):
    ...
    status = Column(
        String(20), default="open", comment="状态: open/assigned/resolved/ignored"
    )
...
class AgentAction(Base):
    ...
    source_id = Column(BigInteger, comment="来源ID")
    exception_id = Column(
        BigInteger, ForeignKey("exception_item.id"), comment="关联异常ID"
    )
    ...
    status = Column(
        String(30), default="proposed", comment="proposed/approved/rejected/executed"
    )
...
class PlatformIntegrationAccount(Base):
    ...
    credential_metadata = Column(
        ...
        default=dict,
        nullable=False,
        comment="密钥元信息: [{key: 'api_key', masked: '***...abcd'}]，不存明文",
    )
    created_by = Column(String(100), comment="创建人")
    updated_by = Column(String(100), comment="更新人")
    created_at = Column(
...
class PlatformCategoryMapping(Base):
    ...
    local_category_id = Column(
        BigInteger, ForeignKey("category.id"), nullable=False, comment="本地类目ID"
    )
...
class CostAllocationBatch(Base):
    ...
    allocation_type = Column(
        String(50), nullable=False, comment="first_leg/fba/overseas_warehouse/other"
    )
    allocation_method = Column(
        String(30), nullable=False, comment="quantity/weight/volume/value"
    )
    ...
    status = Column(
        String(30), server_default="imported", comment="imported/calculated/posted"
    )
...
class CostAllocationItem(Base):
    ...
    batch_id = Column(
        BigInteger,
        ForeignKey("cost_allocation_batch.id"),
        nullable=False,
        comment="批次ID",
    )
    row_number = Column(Integer, nullable=False, comment="原始行号")
...
class User(Base):
    ...
    roles = relationship(
        "Role", secondary="user_role", backref="users", lazy="selectin"
    )
...
class Role(Base):
    ...
    permissions = relationship(
        "Permission", secondary="role_permission", backref="roles", lazy="selectin"
    )
...
class UserRole(Base):
    ...
    role_id = Column(
        BigInteger, ForeignKey("role.id"), nullable=False, comment="角色ID"
    )
...
class RolePermission(Base):
    ...
    permission_id = Column(
        BigInteger, ForeignKey("permission.id"), nullable=False, comment="权限ID"
    )
...
class Order(Base):
    ...
    status = Column(
        String(50),
        default="pending",
        comment="状态: pending/paid/shipped/delivered/completed/cancelled",
    )
...
class OrderShippingSnapshot(Base):
    ...
    sku_id = Column(
        BigInteger, ForeignKey("sku.id"), nullable=False, comment="试算SKU ID"
    )
...
class ImportBatch(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    type = Column(
        String(30), nullable=False, comment="导入类型: product/sku/price/inventory"
    )
...
class PlatformFeeRule(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    platform_id = Column(
        BigInteger, ForeignKey("platform.id"), nullable=False, index=True
    )
    country_code = Column(String(10), comment="国家代码，null表示平台默认")
    category_id = Column(
        BigInteger, ForeignKey("category.id"), comment="类目ID，null表示类目默认"
    )
    fee_type = Column(
        String(30), nullable=False, comment="commission/fixed/payment/storage/other"
    )
...
class Settlement(Base):
    ...
    imported_at = Column(
        DateTime(timezone=True), server_default=func.now(), comment="导入时间"
    )
    created_at = Column(
        DateTime(timezone=True), server_default=func.now(), comment="创建时间"
    )
...
class OrderImport(Base):
    ...
    platform_id = Column(BigInteger, ForeignKey("platform.id"), comment="平台ID")
    source_type = Column(
        String(50), nullable=False, comment="来源: ozon/shopee/wb/manual"
    )
...
class AllocationRule(Base):
    ...
    rule_type = Column(
        String(50), nullable=False, comment="规则类型: percentage/fixed/priority"
    )
    warehouse_id = Column(
        BigInteger, ForeignKey("warehouse.id"), nullable=False, comment="仓库ID"
    )
...
class InventoryWarehouse(Base):
    ...
    __table_args__ = (
        sa_UniqueConstraint("sku_id", "warehouse_id", name="uq_sku_warehouse"),
    )
...
class FinanceAccount(Base):
    ...
    account_type = Column(
        String(50), nullable=False, comment="账户类型: platform/payment/bank/cash"
    )
...
class FinanceTransaction(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    account_id = Column(
        BigInteger, ForeignKey("finance_account.id"), nullable=False, comment="账户ID"
    )
    transaction_type = Column(
        String(50), nullable=False, comment="类型: revenue/cost/fee/refund/transfer"
    )
    ...
    settlement_id = Column(
        BigInteger, ForeignKey("settlement.id"), comment="关联结算单ID"
    )
    platform_id = Column(BigInteger, ForeignKey("platform.id"), comment="关联平台ID")
...
class Notification(Base):
    ...
    alert_type = Column(
        String(50),
        nullable=False,
        comment="类型: inventory_low_stock/inventory_out_of_stock/settlement_pending/settlement_discrepancy/listing_failed/order_pending/system",
    )
...
class ListingTaskItem(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    task_id = Column(
        BigInteger, ForeignKey("listing_task.id"), nullable=False, comment="任务ID"
    )
    product_id = Column(
        BigInteger, ForeignKey("product.id"), nullable=False, comment="商品ID"
    )
    ...
    status = Column(
        String(50),
        default="pending",
        comment="状态: pending/in_progress/success/failed",
    )
    ...
    __table_args__ = (
        sa_UniqueConstraint(
            "task_id", "product_id", "platform_id", name="uq_task_product_platform"
        ),
    )
...
class ProductImageGen(Base):
    ...
    status = Column(
        String(20), default="pending", comment="生成状态: pending/done/failed"
    )
    ...
    batch_id = Column(
        String(36), comment="批量生图的批次标识 (UUID)，便于一次批量结果查询"
    )
    created_at = Column(
        DateTime(timezone=True), server_default=func.now(), comment="创建时间"
    )
...
class ProductCanvas(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    product_id = Column(
        BigInteger, ForeignKey("product.id"), nullable=False, comment="关联商品ID"
    )
...
class AfterSalesOrder(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    order_id = Column(
        BigInteger, ForeignKey("sales_order.id"), nullable=False, comment="原始订单ID"
    )
    item_id = Column(
        BigInteger, ForeignKey("sales_order_item.id"), comment="订单明细ID"
    )
    sku_id = Column(
        BigInteger, ForeignKey("sku.id"), nullable=False, comment="退货SKU ID"
    )
    ...
    status = Column(
        String(30),
        default="pending",
        comment="状态: pending/approved/rejected/received/refunded",
    )
...
class Store(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    user_id = Column(
        BigInteger, ForeignKey("user.id"), nullable=False, comment="所属用户ID"
    )
