...
async def bind_product_supplier(data: ProductSupplierBind, db: AsyncSession = Depends(get_db)):
...
async def get_product_suppliers(product_id: int, db: AsyncSession = Depends(get_db)):
