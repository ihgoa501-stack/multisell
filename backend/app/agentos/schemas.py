...
# ── Phase 6: Policy Matrix Schemas (§2.2) ──────────────────────────


class PolicyCreate(BaseModel):
    """创建冲突仲裁策略"""
    agent_id_a: str = Field(min_length=1, max_length=20, description="冲突参与方A")
    agent_id_b: str = Field(min_length=1, max_length=20, description="冲突参与方B")
    decision_point: str = Field(
        default="*", max_length=50, description="适用的决策点（支持通配符*）"
    )
    condition: Optional[dict[str, Any]] = Field(
        default=None, description="触发条件（可选，如金额范围）"
    )
    winner: str = Field(min_length=1, max_length=20, description="冲突时谁的决策优先")
    reason: str = Field(min_length=1, max_length=500, description="预置理由")
    priority: int = Field(default=100, ge=0, description="匹配优先级")
