...
class TestProviderCRUD:
    ...
    async def test_list_providers(self, async_client):
        """列表物流供应商"""
        await async_client.post("/api/shipping/providers", json={"name": "A", "code": _uc("a")})
        await async_client.post("/api/shipping/providers", json={"name": "B", "code": _uc("b")})
    ...
    async def test_update_provider(self, async_client):
        """更新物流供应商"""
        resp = await async_client.post("/api/shipping/providers", json={"name": "原名称", "code": _uc("orig")})
        pid = resp.json()["data"]["id"]
        resp = await async_client.put(f"/api/shipping/providers/{pid}", json={
            "name": "新名称",
            "contact": "李四",
        })
        assert resp.status_code == 200
    ...
    async def test_delete_provider(self, async_client):
        """删除物流供应商"""
        resp = await async_client.post("/api/shipping/providers", json={"name": "待删除", "code": _uc("del")})
        pid = resp.json()["data"]["id"]
...
class TestChannelCRUD:
    ...
    async def _create_provider(self, async_client, name="测试供应商"):
        code = _uc("test_prov")
        resp = await async_client.post("/api/shipping/providers", json={"name": name, "code": code})
    ...
    async def test_list_channels(self, async_client):
        pid = await self._create_provider(async_client)
        await async_client.post("/api/shipping/channels", json={
            "provider_id": pid, "name": "渠道A", "code": _uc("cha"),
    ...
    async def test_update_channel(self, async_client):
        pid = await self._create_provider(async_client)
        resp = await async_client.post("/api/shipping/channels", json={
            "provider_id": pid, "name": "原名", "code": _uc("orig_c"),
            "volumetric_divisor": 6000, "cargo_types": ["normal"],
        })
        cid = resp.json()["data"]["id"]
        resp = await async_client.put(f"/api/shipping/channels/{cid}", json={
            "name": "新渠道名",
    ...
    async def test_delete_channel(self, async_client):
        pid = await self._create_provider(async_client)
        resp = await async_client.post("/api/shipping/channels", json={
            "provider_id": pid, "name": "待删渠道", "code": _uc("del_c"),
            "volumetric_divisor": 6000, "cargo_types": ["normal"],
        })
        cid = resp.json()["data"]["id"]
...
class TestZoneCRUD:
    ...
    async def _create_channel(self, async_client):
        resp = await async_client.post("/api/shipping/providers", json={"name": "P", "code": _uc("p_zone")})
        pid = resp.json()["data"]["id"]
        resp = await async_client.post("/api/shipping/channels", json={
            "provider_id": pid, "name": "C", "code": _uc("c_zone"),
            "volumetric_divisor": 6000, "cargo_types": ["normal"],
        })
        return resp.json()["data"]["id"]
    ...
    async def test_create_zone(self, async_client):
        ...
        resp = await async_client.post(f"/api/shipping/channels/{cid}/zones", json={
            "country_code": "US",
    ...
    async def test_list_zones(self, async_client):
        cid = await self._create_channel(async_client)
        await async_client.post(f"/api/shipping/channels/{cid}/zones", json={"country_code": "US"})
    ...
    async def test_delete_zone(self, async_client):
        cid = await self._create_channel(async_client)
        resp = await async_client.post(f"/api/shipping/channels/{cid}/zones", json={"country_code": "DE"})
...
class TestRuleCRUD:
    ...
    async def _create_channel(self, async_client):
        resp = await async_client.post("/api/shipping/providers", json={"name": "P2", "code": _uc("p_rule")})
        pid = resp.json()["data"]["id"]
        resp = await async_client.post("/api/shipping/channels", json={
            "provider_id": pid, "name": "C2", "code": _uc("c_rule"),
            "volumetric_divisor": 6000, "cargo_types": ["normal"],
        })
        return resp.json()["data"]["id"]
    ...
    async def test_create_rule(self, async_client):
        cid = await self._create_channel(async_client)
        resp = await async_client.post(f"/api/shipping/channels/{cid}/rules", json={
    ...
    async def test_update_rule(self, async_client):
        ...
        resp = await async_client.put(f"/api/shipping/rules/{rid}", json={
            "rule_type": "tiered_weight",
    ...
    async def test_delete_rule(self, async_client):
        cid = await self._create_channel(async_client)
        resp = await async_client.post(f"/api/shipping/channels/{cid}/rules", json={
            "rule_type": "fixed_plus_per_kg", "fixed_fee": 5, "per_kg_price": 30,
        })
        rid = resp.json()["data"]["id"]
...
class TestPermissions:
    ...
    async def test_no_token_returns_401(self, async_client):
        """未登录返回 401"""
        resp = await async_client.post("/api/shipping/providers", json={"name": "X", "code": _uc("x")})
    ...
    async def test_no_permission_returns_403(self, async_client):
        ...
        resp = await async_client.post("/api/shipping/providers", json={"name": "X", "code": _uc("x")}, headers=headers)
    ...
    async def test_granted_manage_can_write(self, async_client):
        ...
        headers = {"Authorization": f"Bearer {token}"}
        resp = await async_client.post("/api/shipping/providers", json={"name": "可管理", "code": _uc("mgmt")}, headers=headers)
        assert resp.status_code == 200
    ...
    async def test_calculate_requires_permission(self, async_client):
        ...
        try:
            ...
            assert resp.status_code == 200
            pid = resp.json()["data"]["id"]
            resp = await async_client.post(f"/api/products/{pid}/specs", json={
        ...
        resp = await async_client.post("/api/shipping/calculate", json={
            "sku_id": sid, "quantity": 1, "destination_country": "US", "cargo_type": "normal",
        }, headers=headers)
        assert resp.status_code == 200
        assert resp.json()["code"] == 200
    ...
    async def test_write_creates_operation_log(self, async_client):
        ...
        await grant_permission(uid, "shipping:manage")
        headers = {"Authorization": f"Bearer {token}"}
        resp = await async_client.post("/api/shipping/providers", json={"name": "审计测试", "code": _uc("audit_p")}, headers=headers)
        assert resp.status_code == 200
        ...
        resp = await async_client.get("/api/operation-logs?module=shipping_provider", headers={"Authorization": f"Bearer {token_view}"})
        assert resp.status_code == 200
