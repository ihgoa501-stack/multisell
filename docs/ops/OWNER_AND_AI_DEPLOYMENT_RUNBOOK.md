...
这台服务器用于 Owner 自用的凌镜跨境经营实验系统。服务器不预设当前市场、平台、类目、商品或候选数量。只有候选市场通过比较并由 Owner 批准后，才能启用该市场所需的平台连接器：

```text
候选市场比较 → Owner 批准已选市场 → 需求与反证 → 最小真实实验 → 最终有效成交 → 最终贡献利润与现金回收
```
...
- 未经 Owner 批准执行任何平台发布、调价、库存、采购、广告、订单、退款或资金动作；
...
首次部署允许暂时不配置 LLM 和任何外部平台密钥。缺失密钥时应保持 read-only、stub 或功能关闭，不得伪造已接通。平台密钥只能在对应市场通过选择闸门后按需配置。
...
应用回滚必须回到明确的 Git 提交或镜像版本，并同步回滚后端、前端和图片服务；图片服务迁移器只允许执行目标版本的向上迁移。主数据库已进入 migration 152 后，应用回滚必须保留数据库 152，禁止通用 `migrate down`、`goto` 或 `--revert-migration`。如果确实需要恢复主数据库，必须停止全部写入、取得 Owner 明确批准，并使用已经验证过的异地不可变迁移前备份整库恢复；不得用 152 的 down migration 恢复已知分叉链。
