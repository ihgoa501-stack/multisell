...
function extractFromLDJSON(parsed) {
    ...
    if (parsed.image) {
        ...
        if (Array.isArray(img))
            data.images = img.filter((i) => typeof i === "string");
    }
    ...
    try {
        ...
        if (offer.priceRange && Array.isArray(offer.priceRange)) {
            const prices = offer.priceRange
                .map((p) => parseFloat(p.price || p))
                .filter((p) => !isNaN(p));
            if (prices.length > 0) {
                data.price_min = Math.min(...prices);
                data.price_max = Math.max(...prices);
                if (!data.price_1688)
                    data.price_1688 = prices[0];
            }
        }
        if (offer.images && Array.isArray(offer.images)) {
            data.images = offer.images.map((img) => typeof img === "string" ? img : img.url || img.imageUrl || "").filter(Boolean);
        }
...
function extractSKUData(skuData) {
    ...
    try {
        if (Array.isArray(skuData)) {
            ...
            for (const [key, val] of Object.entries(skuData)) {
                variants.push({
                    spec: key,
...
function extractSpecVariantsFromDOM() {
    ...
    if (variants.length === 0) {
        ...
        for (const sel of specSelectors) {
            ...
            if (items.length > 0) {
                items.forEach((item) => {
                    const spec = item.innerText?.trim() || "";
...
function extractSupplierFromDOM() {
    ...
    for (const sel of scoreSelectors) {
        const el = document.querySelector(sel);
        if (el) {
            const text = el.innerText?.trim() || el.getAttribute("data-score") || "";
            const match = text.match(/(\d+\.?\d*)/);
            if (match) {
...
function extractMOQFromDOM() {
    const bodyText = document.body.innerText;
    const moqMatch = bodyText.match(/(?:起订量|最小起订|MOQ|min\s*order)[：:]\s*(\d+)/i);
    if (moqMatch)
        return parseInt(moqMatch[1], 10);
...
function extractPackageFromDOM() {
    ...
    const wMatch = bodyText.match(/(?:毛重|重量|净重|package\s*weight)[：:]\s*(\d+\.?\d*)\s*(kg|公斤|g)/i);
    if (wMatch) {
        result.weight = parseFloat(wMatch[1]);
        if (wMatch[2].toLowerCase() === "g")
            result.weight /= 1000;
    }
    // Dimensions: 包装尺寸, 外箱尺寸, volume, package size
    const dMatch = bodyText.match(/(?:包装尺寸|外箱尺寸|体积|package\s*size|dimensions?)[：:]\s*(\d+\.?\d*)\s*[*×xX]\s*(\d+\.?\d*)\s*[*×xX]\s*(\d+\.?\d*)/i);
    if (dMatch) {
...
function extractPageData() {
    ...
    try {
        ...
        if (embedded) {
            ...
            if (embedded.title)
                data.title = embedded.title;
            if (embedded.price_1688 && embedded.price_1688 > 0)
                data.price_1688 = embedded.price_1688;
            if (embedded.images && embedded.images.length > 0)
                data.images = embedded.images;
            ...
            if (embedded.supplier_name)
                data.supplier_name = embedded.supplier_name;
            if (embedded.supplier_id_1688)
                data.supplier_id_1688 = embedded.supplier_id_1688;
            if (embedded.description)
                data.description = embedded.description;
            if (embedded.price_min !== undefined)
                data.price_min = embedded.price_min;
            if (embedded.price_max !== undefined)
                data.price_max = embedded.price_max;
        }
    }
    catch {
