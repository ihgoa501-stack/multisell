...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_settlement_batch_status", "platform_settlement_batch", ["status"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "batch_id",
            sa.BigInteger(),
            sa.ForeignKey("platform_settlement_batch.id"),
            nullable=False,
        ),
    ...
    op.create_index(
        "ix_settlement_item_batch_id", "platform_settlement_item", ["batch_id"]
    )
    op.create_index(
        "ix_settlement_item_match_status", "platform_settlement_item", ["match_status"]
    )


def downgrade() -> None:
    op.drop_index(
        "ix_settlement_item_match_status", table_name="platform_settlement_item"
    )
