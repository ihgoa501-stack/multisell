...
// ImportSyncStatus tracks per-platform order sync status.
type ImportSyncStatus struct {
	ID             uint       `gorm:"primaryKey" json:"id"`
	Platform       string     `gorm:"size:50;uniqueIndex:idx_import_sync_platform" json:"platform"`
	LastSyncAt     *time.Time `json:"last_sync_at,omitempty"`
	LastSyncResult string     `gorm:"size:20" json:"last_sync_result"`
...
func (ImportSyncStatus) TableName() string { return "import_sync_status" }

// SyncOrderInput describes a single order to import idempotently.
type SyncOrderInput struct {
	Platform        string  `json:"platform"`
	PlatformOrderNo string  `json:"platform_order_no" binding:"required"`
...
// RetryInput is the body for POST /order-import/:id/retry.
