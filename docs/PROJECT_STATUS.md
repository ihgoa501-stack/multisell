...
已实现：
- 导入平台结算 CSV，支持 sale / platform_fee / payment_fee / refund / adjustment / payout / tax / other 交易类型。
- 按 order_no 自动匹配订单，已匹配标记 matched，未匹配标记 unmatched。
- 结算行返回 cost_layer = actual。
- 前端导入、批次列表、明细查询页面。
- 操作写入审计日志。
...
已实现：
- FinanceLedgerEntry 模型记录每笔订单的利润条目。
- 重建账本时从订单、运费快照、物流账单、平台结算行自动汇总。
- 运费按优先级取实际账单 > 快照 > 订单默认。
- 平台费优先取结算行，无结算行时使用订单默认。
- 利润成本层按来源自动计算 estimated / snapshot / actual / mixed。
- 账本重建、查看接入权限和审计日志。
...
已实现：
- 扫描 listing blocked/failed、shipping bill 异常、settlement unmatched、订单负利润 4 类来源生成异常。
- 异常支持分配、解决、忽略。
- 异常列表支持按模块、严重程度、状态筛选。
- 异常生成、管理接入权限和审计日志。
...
已实现：
- AgentAction 模型记录动作提案、审批、执行的全生命周期。
- 状态流：proposed → approved → executed 或 proposed → rejected。
- 支持 before/after snapshot，用于审计动作前后状态变化。
- 异常工作台已集成 Agent 动作面板，可在异常详情中创建和审批提案。
- 4 级权限：propose / view / approve / execute，所有操作写入审计日志。

暂未实现：
- 真实 LLM 集成。
- 自动执行（agent 不自动调用外部 API）。
- 通知系统。
...
已实现：
- 利润摘要：总收入、商品成本、运费、平台费、支付费、退款、分摊成本、其他费用、利润、利润率。
- 订单利润列表：每单展示收入/成本/费用/利润/利润率 + 成本层来源。
- 运费差异报表：快照运费 vs 实际账单运费的差异金额和百分比。
- 负利润订单列表：利润小于 0 的订单及其成本层来源。
- 成本层分布统计：按 estimated / snapshot / actual / allocated / mixed 分组统计条目数和金额。
- 所有报表支持 date_from / date_to 时间范围筛选。
- 前端利润看板页面含摘要卡片、成本层分布表、运费差异表、负利润表。
- 报表查看接入 finance:report:view 权限。
...
状态：Stage 11 已完成，Stage 12 经营链路已完成。
...
- 同一 `platform_order_no` 多 SKU 合并成一个订单（包含第二 SKU 的 OrderItem 创建、库存锁定、金额重算）。
- 同一批次内多 SKU 正确合并，无效 SKU 标记 failed 不影响有效 SKU 创建订单。
...
Stage 12 已实现：
- `POST /api/order-imports/{batch_id}/process-chain` 处理批次经营链路。
- `GET /api/order-imports/{batch_id}/chain-summary` 查询链路状态摘要。
- 链路处理：对导入订单执行 `LedgerService.rebuild()` 重建利润账本。
- 链路处理：调用 `ExceptionService.generate()` 生成异常工作台条目。
- 批次和明细新增 `chain_status`、`ledger_rebuilt_count`、`exception_generated_count`、`chain_failure_count`、`processed_at` 字段。
- 新增权限码：`order_import:process`。
