...
printf '%s\n' "$cert" | openssl x509 -checkend "$WARN_SECONDS" -noout >/dev/null || { echo "IP certificate expires within ${WARN_SECONDS} seconds" >&2; exit 1; }
printf '%s\n' "$cert" | openssl x509 -noout -ext subjectAltName | grep -q "IP Address:${PUBLIC_IP}" || { echo "certificate SAN does not match ${PUBLIC_IP}" >&2; exit 1; }
