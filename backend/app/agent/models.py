...
class AgentDecision(Base):
    ...
    agent_id = Column(
        String(20), nullable=False, comment="Agent标识: A3/A4/A5/A6/A7/G1/G2/G3"
    )
    decision_point = Column(
        String(50),
        nullable=False,
        comment="决策点: acos_adjustment/stock_alert/discount_check 等",
    )
    ...
    user_action = Column(
        String(20),
        nullable=False,
        comment="用户操作: accepted/modified/rejected/ignored",
    )
    ...
    evolution_stage = Column(
        String(20),
        nullable=False,
        comment="进化阶段: observation/suggestion/semi_autonomous/full_autonomous",
    )
...
class PersonalRule(Base):
    ...
    rule_type = Column(
        String(20), nullable=False, comment="规则类型: threshold/strategy/style/veto"
    )
    ...
    source = Column(
        String(20), nullable=False, comment="来源: manual/nudge/auto_extracted/template"
    )
    source_decisions = Column(JSON, comment="来源决策ID列表")
    status = Column(
        String(20), default="active", comment="状态: active/shadow/paused/retired"
    )
...
class HonchoProfile(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    user_id = Column(
        BigInteger, ForeignKey("user.id"), nullable=False, unique=True, comment="用户ID"
    )

    risk_tolerance = Column(
        String(30),
        default="moderate",
        comment="风险容忍度: conservative/moderate/aggressive",
    )
    communication_style = Column(
        String(20), default="balanced", comment="沟通风格: concise/balanced/detailed"
    )
    notification_prefs = Column(JSON, comment="通知偏好")
    agent_profiles = Column(
        JSON, nullable=False, default=lambda: {}, comment="各Agent配置档案"
    )
...
class AgentAction(Base):
    ...
    agent_id = Column(String(20), nullable=False, comment="Agent标识")
    decision_id = Column(
        BigInteger, ForeignKey("agent_decision.id"), comment="关联决策ID"
    )
    action_type = Column(
        String(50),
        nullable=False,
...
class RuleConflict(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    decision_id = Column(
        BigInteger, ForeignKey("agent_decision.id"), nullable=False, comment="决策ID"
    )
    ...
    resolution = Column(
        String(20),
        nullable=False,
        comment="解决方式: auto_priority/user_choice/latest_wins",
    )
...
class SpcControlLimit(Base):
    ...
    metric_name = Column(
        String(50),
        nullable=False,
        comment="指标名: acceptance_rate/confidence/override_rate",
    )
    ...
    baseline_recalc_at = Column(
        DateTime(timezone=True), nullable=False, comment="基线计算时间"
    )
    next_recalc_at = Column(
        DateTime(timezone=True), nullable=False, comment="下次重算时间"
    )
    ...
    __table_args__ = (
        sa_UniqueConstraint(
            "user_id", "agent_id", "decision_point", "metric_name", name="uq_spc_metric"
        ),
    )
...
class AgentEvolutionConfig(Base):
    ...
    current_stage = Column(
        String(20), nullable=False, default="observation", comment="当前自治阶段"
    )
    stage_updated_at = Column(DateTime(timezone=True), comment="阶段最后变更时间")
    stage_updated_by = Column(
        String(20), default="system", comment="变更来源: manual/nudge/regret/system"
    )
    ...
    __table_args__ = (
        sa_UniqueConstraint(
            "user_id", "agent_id", "decision_point", name="uq_agent_evolution"
        ),
    )
...
class AgentNudge(Base):
    ...
    trust_score_at_time = Column(
        Numeric(6, 2), nullable=False, comment="提示时的信任评分"
    )
    ...
    status = Column(
        ...
        nullable=False,
        default="pending",
        comment="状态: pending/accepted/dismissed/expired",
    )
