...
Owner浏览1688首页、搜索结果或商品详情页时，可以主动选择当前可见商品或点击“采集到凌镜”，商品经HTTPS保存到Owner私人采集箱。商品初始状态为`unverified_lead`，只表示Owner保存过该页面及页面当时的声明，不代表商品机会、可信供应商或可上架草稿。
...
```text
打开1688首页或搜索结果页
→ 勾选当前可见商品并点击“采集选中”，或点击“采集本页”
→ 列表页先保存可靠取得的标题、报价、图片和详情链接
→ 需要完整SKU、MOQ和供应商时再打开商品详情页补充采集
...
代码更新后，在 `chrome://extensions` 找到凌镜插件并点击刷新，然后刷新已经打开的1688页面。首页/搜索页显示独立的列表采集面板；详情页采集面板可拖动，松开后自动吸附左右两侧，并记住上次位置。
...
扩展不申请淘宝、Ozon、浏览历史、Cookie、下载或全部网站权限。列表页只识别当前已加载且可见的精确1688商品详情链接，不自动翻页、不采完整店铺，也不设置固定商品数量。Manifest权限变化由 `tests/manifest.test.mjs` 检查。
...
- 固定schema的有限页面结构化数据及原始字节；私人收藏不上传DOM片段、整页HTML或页面脚本；
...
| 提示 | 含义与处理 |
|---|---|
| 当前页没有可靠商品 | 在首页/搜索页等待商品卡加载，或打开 `detail.1688.com/offer/...` 后重试 |
...
自动测试证明编译、请求映射、成功回执规则、Manifest范围和无自动上传路径；它不能证明真实1688页面字段已经准确。真实可用仍需Owner完成一个真实首页/搜索页列表批次和一个真实详情商品的Chrome主闭环，并逐项人工对照。
