...
async def seed_inventory_details(session):
    ...
    for sku in skus:
        ...
        print(
            f"     ✔ SKU {sku.code}: 锁定={inv.locked_quantity}, 安全库存={inv.safety_stock}"
        )
            ...
            for dec in decisions:
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        print(
                            f"     ✔ {agent_id}/{dec['decision_point']} → id={decision_id}"
                        )
                    else:
                        print(
                            f"     ⚠ {agent_id}/{dec['decision_point']} → {resp.status_code}"
                        )
