...
async def handle_profit_review(db: AsyncSession, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if sku_id is None or target_sale_price is None:
        raise ValueError(f"profit_review 需要 sku_id 和 target_sale_price, 收到 payload keys: {list(payload.keys())}")
...
async def handle_listing_draft(db: AsyncSession, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if platform_id:
        ...
        if listing:
            ...
            logger.warning("listing_draft: _get_or_create_listing returned None for product=%s platform=%s", product_id, platform_id)
