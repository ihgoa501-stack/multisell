...
func RegisterRoutes(rg *gin.RouterGroup, db *gorm.DB, logger *zap.Logger, bridge toolbridge.Bridge) {
	...
	group := rg.Group("/sourcing-1688", middleware.RequirePermission(db, "product.read"))
	{
		...
		group.GET("/:id/collection-quality", h.CollectionQuality)
		group.GET("/:id/draft", h.Draft)
		group.GET("/:id/identity-history", h.IdentityHistory)
		group.GET("/:id/watch", h.GetWatch)
		group.GET("/:id/watch/alerts", h.ListWatchAlerts)
		group.GET("/:id/watch/refresh-runs", h.ListWatchRuns)
		group.GET("/:id/watch/refresh-runs/:runId", h.GetWatchRun)
		...
		group.GET("/:id/task-links/:linkId/sku-mappings", h.ListCanonicalSKUMappings)
		group.GET("/:id/task-links/:linkId/sku-workspace", h.SKUWorkspace)
		group.GET("/:id/task-links/:linkId/material-assets", h.ListMaterialAssets)
	...
	write.PUT("/:id/watch", h.SetWatch)
	write.POST("/:id/watch/refresh-runs", h.CreateWatchRun)
	write.POST("/:id/watch/refresh-runs/:runId/evaluate", h.EvaluateWatchRun)
	...
	write.POST("/:id/task-links/:linkId/material-assets", h.CreateMaterialAsset)
	write.PATCH("/:id/task-links/:linkId/material-assets/:assetId/order", h.ReorderMaterialAsset)
	write.POST("/:id/task-links/:linkId/material-assets/:assetId/archive", h.ArchiveMaterialAsset)
	write.POST("/:id/task-links/:linkId/material-assets/:assetId/mark-used", h.MarkMaterialUsed)
	write.POST("/:id/task-links/:linkId/material-assets/:assetId/rights-evidence", h.AddMaterialRights)
	write.POST("/:id/task-links/:linkId/material-assets/:assetId/rights-evidence/:evidenceId/review", h.ReviewMaterialRights)
	write.POST("/:id/task-links/:linkId/material-assets/:assetId/rights-evidence/:evidenceId/revoke", h.RevokeMaterialRights)
	write.POST("/:id/task-links/:linkId/material-assets/:assetId/renditions", h.AttachMaterialRendition)
