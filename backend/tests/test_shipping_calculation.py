...
async def _ensure_seed(async_client, country: str):
    ...
    resp = await async_client.post("/api/products", json={
        "name": f"Test_{uid}", "package_length_cm": 30, "package_width_cm": 20,
        "package_height_cm": 10, "package_weight_kg": 0.8, "cargo_type": "normal",
    })
    assert resp.status_code == 200
    pid = resp.json()["data"]["id"]

    await async_client.post(f"/api/products/{pid}/specs", json={"specs": [{"name": "颜色", "values": ["红"]}]})
    ...
    resp = await async_client.post("/api/shipping/providers", json={
        "name": f"P_{uid}", "code": f"p_{uid}",
    })
    provid = resp.json()["data"]["id"]
    resp = await async_client.post("/api/shipping/channels", json={
        "provider_id": provid, "name": f"C_{uid}", "code": f"c_{uid}",
        "volumetric_divisor": 6000, "cargo_types": ["normal"],
    })
    cid = resp.json()["data"]["id"]
    await async_client.post(f"/api/shipping/channels/{cid}/zones", json={"country_code": country})
    await async_client.post(f"/api/shipping/channels/{cid}/rules", json={
        "rule_type": "fixed_plus_per_kg", "fixed_fee": 8, "per_kg_price": 42,
        "minimum_charge": 25, "rounding_increment": 0.1,
    })
...
async def _seed_channel(async_client, country: str, rule_data: dict) -> tuple:
    ...
    uid = uuid4().hex[:6]
    resp = await async_client.post("/api/shipping/providers", json={
        "name": f"S_{uid}", "code": f"s_{uid}",
    })
    assert resp.status_code == 200, f"create provider failed: {resp.text}"
    pid = resp.json()["data"]["id"]

    resp = await async_client.post("/api/shipping/channels", json={
        "provider_id": pid, "name": f"Ch_{uid}", "code": f"ch_{uid}",
    ...
    rule = {k: v for k, v in rule_data.items() if k not in ("volumetric_divisor", "cargo_types")}
...
class TestPackageResolution:
    ...
    async def test_product_package_fallback(self, async_client):
        ...
        resp = await async_client.post("/api/shipping/calculate", json={
            "sku_id": d["sku_id"], "quantity": 1,
            "destination_country": c, "cargo_type": "normal",
        })
        body = resp.json()
    ...
    async def test_sku_override(self, async_client):
        c = _unique_country()
        d = await _ensure_seed(async_client, c)
        await async_client.put(f"/api/skus/{d['sku_id']}", json={
            "sku_length_cm": 25, "sku_width_cm": 15,
            "sku_height_cm": 8, "sku_weight_kg": 0.5,
        })
        resp = await async_client.post("/api/shipping/calculate", json={
            "sku_id": d["sku_id"], "quantity": 1,
            "destination_country": c, "cargo_type": "normal",
    ...
    async def test_missing_package_data_blocks_calculation(self, async_client):
        ...
        resp = await async_client.post("/api/shipping/calculate", json={
            "sku_id": sid, "quantity": 1,
            "destination_country": "US", "cargo_type": "normal",
        })
...
class TestWeightFormula:
    ...
    async def test_actual_weight_greater_than_volumetric(self, async_client):
        c = _unique_country()
        _, cid = await _seed_channel(async_client, c, {
            "volumetric_divisor": 6000, "fixed_fee": 5, "per_kg_price": 20,
        })
        sid = await _seed_product_sku(async_client, {
            "package_length_cm": 10, "package_width_cm": 10,
            "package_height_cm": 10, "package_weight_kg": 5.0,
        })
        resp = await async_client.post("/api/shipping/calculate", json={
            "sku_id": sid, "quantity": 1,
            "destination_country": c, "cargo_type": "normal",
    ...
    async def test_rounding(self, async_client):
        c = _unique_country()
        _, cid = await _seed_channel(async_client, c, {
            "volumetric_divisor": 5000, "fixed_fee": 10, "per_kg_price": 30,
            "rounding_increment": 0.5,
        })
        sid = await _seed_product_sku(async_client, {
            "package_length_cm": 20, "package_width_cm": 15,
            "package_height_cm": 10, "package_weight_kg": 0.6,
        })
        resp = await async_client.post("/api/shipping/calculate", json={
            "sku_id": sid, "quantity": 1,
            "destination_country": c, "cargo_type": "normal",
...
class TestQuoteRuleTypes:
    ...
    async def test_first_weight_plus_increment(self, async_client):
        c = _unique_country()
        _, cid = await _seed_channel(async_client, c, {
            "volumetric_divisor": 8000, "rule_type": "first_weight_plus_increment",
            "first_kg": 0.1, "first_price": 20, "additional_kg": 0.1, "additional_price": 5,
        })
        sid = await _seed_product_sku(async_client, {
            "package_length_cm": 20, "package_width_cm": 15,
            "package_height_cm": 5, "package_weight_kg": 0.35,
        })
        resp = await async_client.post("/api/shipping/calculate", json={
            "sku_id": sid, "quantity": 1,
            "destination_country": c, "cargo_type": "normal",
    ...
    async def test_tiered_weight(self, async_client):
        c = _unique_country()
        _, cid = await _seed_channel(async_client, c, {
            "volumetric_divisor": 5000, "rule_type": "tiered_weight",
            "tier_config": [
        ...
        sid = await _seed_product_sku(async_client, {
            "package_length_cm": 25, "package_width_cm": 20,
            "package_height_cm": 15, "package_weight_kg": 1.2,
        })
        resp = await async_client.post("/api/shipping/calculate", json={
            "sku_id": sid, "quantity": 1,
            "destination_country": c, "cargo_type": "normal",
...
class TestSurchargesAndMinimum:
    ...
    async def test_minimum_charge_applied(self, async_client):
        c = _unique_country()
        _, cid = await _seed_channel(async_client, c, {
            "fixed_fee": 8, "per_kg_price": 42, "minimum_charge": 25,
        })
        sid = await _seed_product_sku(async_client, {
            "package_length_cm": 5, "package_width_cm": 5,
            "package_height_cm": 3, "package_weight_kg": 0.05,
        })
        resp = await async_client.post("/api/shipping/calculate", json={
            "sku_id": sid, "quantity": 1,
            "destination_country": c, "cargo_type": "normal",
    ...
    async def test_fixed_surcharge(self, async_client):
        c = _unique_country()
        _, cid = await _seed_channel(async_client, c, {
            "cargo_types": ["battery"], "fixed_fee": 10, "per_kg_price": 50,
            "minimum_charge": 30, "surcharge_fixed": 15, "fuel_surcharge_pct": 15,
        })
        sid = await _seed_product_sku(async_client, {
            "package_length_cm": 20, "package_width_cm": 15,
        ...
        resp = await async_client.post("/api/shipping/calculate", json={
            "sku_id": sid, "quantity": 1,
            "destination_country": c, "cargo_type": "battery",
        })
        r = [x for x in resp.json()["data"]["results"] if x["channel_id"] == cid]
        assert len(r) == 1, f"channel {cid} not found"
        r = r[0]
...
class TestFiltering:
    ...
    async def test_inactive_provider_excluded(self, async_client):
        c = _unique_country()
        d = await _ensure_seed(async_client, c)
        await async_client.put(f"/api/shipping/providers/{d['provider_id']}", json={"status": 0})
        resp = await async_client.post("/api/shipping/calculate", json={
            "sku_id": d["sku_id"], "quantity": 1,
            "destination_country": c, "cargo_type": "normal",
        })
    ...
    async def test_destination_country_mismatch_excluded(self, async_client):
        ...
        resp = await async_client.post("/api/shipping/calculate", json={
            "sku_id": d["sku_id"], "quantity": 1,
            "destination_country": unmatched_country, "cargo_type": "normal",
        })
        assert len(resp.json()["data"]["results"]) == 0
    ...
    async def test_results_sorted_by_total_fee(self, async_client):
        ...
        uid = uuid4().hex[:6]
        resp = await async_client.post("/api/shipping/channels", json={
            "provider_id": d["provider_id"], "name": f"Eco_{uid}", "code": f"eco_{uid}",
            "volumetric_divisor": 8000, "cargo_types": ["normal"],
        })
        assert resp.status_code == 200
        cid2 = resp.json()["data"]["id"]
        await async_client.post(f"/api/shipping/channels/{cid2}/zones", json={"country_code": c})
        await async_client.post(f"/api/shipping/channels/{cid2}/rules", json={
            "rule_type": "fixed_plus_per_kg", "fixed_fee": 5, "per_kg_price": 30, "rounding_increment": 0.1,
        })
        ...
        assert len(results) == 2, f"expected 2 results, got {len(results)}: {resp.json()}"
