...
// UpgradeResult describes one agent autonomy upgrade.
...
// Upgrader handles autonomy level upgrades based on trust scores.
...
// UpgradeEligible checks all agents and upgrades those eligible.
func (u *Upgrader) UpgradeEligible() ([]UpgradeResult, error) {
	var results []UpgradeResult
	agents := defaultAgentList()
	for _, agent := range agents {
		score, err := getScoreByAgent(u.db, agent.ID)
		if err != nil || score == nil {
			continue
		}
		target := determineTargetLevel(score.TrustScore, score.AutonomyLevel)
		if target != score.AutonomyLevel {
			if err := u.db.Model(score).Update("autonomy_level", target).Error; err != nil {
				u.logger.Warn("upgrade failed", zap.String("agent", agent.ID), zap.Error(err))
				continue
			}
...
func getScoreByAgent(db *gorm.DB, agentID string) (*TrustScore, error) {
	var score TrustScore
	if err := db.Where("agent_id = ?", agentID).First(&score).Error; err != nil {
		return nil, err
	}
	return &score, nil
}
