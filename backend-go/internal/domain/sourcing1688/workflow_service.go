...
func (s *Service) Capture(in *CaptureInput) (*Sourcing1688Product, error) {
	...
	if in.DemandCaseID == 0 || in.ExperimentID == "" || in.CollectedBy == 0 || in.CollectedAt.IsZero() || strings.TrimSpace(in.Driver) == "" || strings.TrimSpace(in.ParserVersion) == "" || len(in.RawPayload) == 0 || !json.Valid(in.RawPayload) {
	...
	err = s.db.Transaction(func(tx *gorm.DB) error {
		...
		if errors.Is(err, gorm.ErrRecordNotFound) {
			p = Sourcing1688Product{SourceURL: canonicalURL, SourceOfferID: offerID, Title: in.Title, Price: in.Price, SupplierName: in.SupplierName, Status: StatusPendingReview, DemandCaseID: &in.DemandCaseID, ExperimentID: &in.ExperimentID}
			if in.MOQ != nil {
				p.MOQ = *in.MOQ
			} else {
				p.MOQ = 1
...
func (s *Service) Review(id int64, in *ReviewInput) (*Sourcing1688Product, error) {
	...
	err := s.db.Transaction(func(tx *gorm.DB) error {
		...
		now := time.Now().UTC()
		if err := tx.Model(&p).Updates(map[string]any{"status": StatusReviewed, "reviewed_by": in.ReviewedBy, "reviewed_at": now, "review_notes": strings.TrimSpace(in.Notes)}).Error; err != nil {
			return err
		}
...
func validateConvert(in *ConvertInput) error {
	...
	if err := validateChecks(in.SupplierAssessment, []string{"identity", "operating_history", "transaction_history", "mixed_batch", "lead_time", "sample", "returns"}); err != nil {
		return fmt.Errorf("%w: supplier assessment: %v", ErrWorkflowGate, err)
	}
	if err := validateChecks(in.ComplianceChecks, []string{"brand_ip", "patent", "certification", "dangerous_goods", "material", "labeling_instructions"}); err != nil {
	...
	hasMain := false
	required := map[string]bool{"purchase": false, "domestic_shipping": false, "packaging": false, "cross_border_shipping": false, "platform_fee": false, "payment_fee": false, "advertising": false, "tax": false, "duty": false, "return_loss": false, "exchange_rate": false}
...
func validateChecks(checks []EvidenceCheck, required []string) error {
	...
	for _, check := range checks {
		if !allowed[check.CheckType] || seen[check.CheckType] || check.Result != "pass" || (check.TruthStatus != "actual" && check.TruthStatus != "quoted") || strings.TrimSpace(check.SourceURI) == "" || check.ObservedAt.IsZero() {
...
func (s *Service) Convert(id int64, in *ConvertInput) (*ConvertResult, error) {
	...
	err := s.db.Transaction(func(tx *gorm.DB) error {
		...
		if p.Status != StatusReviewed || p.SnapshotID == nil || p.DemandCaseID == nil || p.ExperimentID == nil || p.ReviewedBy == nil || *p.ReviewedBy != in.CreatedBy {
		...
		for _, v := range in.SKUVariants {
			row := skuRow{ProductID: prod.ID, Code: v.ChannelSKU, SpecDesc: v.SpecDesc, SpecValues: v.SpecValues, Price: v.Price, CostPrice: v.CostPrice, Weight: v.Weight, Image: v.Image, Status: 1}
		...
		if err := tx.Model(&p).Updates(map[string]any{"status": StatusDraftCreated, "product_id": prod.ID, "imported_by": strconv.FormatInt(in.CreatedBy, 10), "imported_at": time.Now().UTC()}).Error; err != nil {
			return err
		}
