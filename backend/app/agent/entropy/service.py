...
from app.agent.entropy.defenses import (
    TTLSweeper,
    BudgetEnforcer,
    DecayScheduler,
    MergeDetector,
...
class EntropyService:
    ...
    async def get_change_log(
        ...
        stmt = (
            stmt.order_by(RuleMarkChange.created_at.desc())
            .offset(offset)
            .limit(page_size)
        )
        result = await db.execute(stmt)
