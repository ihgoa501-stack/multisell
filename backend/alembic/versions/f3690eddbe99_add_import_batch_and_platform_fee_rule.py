...
def upgrade() -> None:
    op.create_table('import_batch',
        ...
        sa.Column('type', sa.String(length=30), nullable=False, comment='导入类型: product/sku/price/inventory'),
        sa.Column('file_name', sa.String(length=255), nullable=True, comment='原始文件名'),
        sa.Column('status', sa.String(length=20), nullable=False, server_default='pending', comment='pending/previewed/committed/failed'),
    ...
    op.create_table('import_batch_row',
        sa.Column('id', sa.BigInteger(), autoincrement=True, nullable=False),
        sa.Column('batch_id', sa.BigInteger(), nullable=False),
        sa.Column('row_index', sa.Integer(), nullable=False, comment='Excel行号'),
        sa.Column('status', sa.String(length=20), nullable=False, server_default='pending', comment='pending/success/error'),
    ...
    op.create_index(op.f('ix_import_batch_row_batch_id'), 'import_batch_row', ['batch_id'], unique=False)
    ...
    op.create_table('platform_fee_rule',
        sa.Column('id', sa.BigInteger(), autoincrement=True, nullable=False),
        sa.Column('platform_id', sa.BigInteger(), nullable=False),
        sa.Column('country_code', sa.String(length=10), nullable=True, comment='国家代码，null表示平台默认'),
        sa.Column('category_id', sa.BigInteger(), nullable=True, comment='类目ID，null表示类目默认'),
        sa.Column('fee_type', sa.String(length=30), nullable=False, comment='commission/fixed/payment/storage/other'),
        ...
        sa.Column('status', sa.String(length=20), default='active', comment='active/inactive'),
    ...
    op.create_index(op.f('ix_platform_fee_rule_platform_id'), 'platform_fee_rule', ['platform_id'], unique=False)


def downgrade() -> None:
    op.drop_index(op.f('ix_platform_fee_rule_platform_id'), table_name='platform_fee_rule')
