...
func OpenStore(path string) (*Store, error) {
	s := &Store{path: path, jobs: map[string]*Job{}, attempts: map[string]*Attempt{}, consumedNonces: map[string]time.Time{}}
...
func (s *Store) Create(in CreateJob) (*Job, bool, error) {
	...
	j := &Job{ID: id, OwnerID: in.OwnerID, LingMirrorTaskID: in.LingMirrorTaskID, LingMirrorTaskVersion: in.LingMirrorTaskVersion, IdempotencyKey: in.IdempotencyKey, ManifestHash: in.ManifestHash, Operation: in.Operation, Processor: processor, Prompt: in.Prompt, InputBlobID: in.InputBlobID, Width: in.Width, Height: in.Height, Format: in.Format, Status: JobQueued, Version: 1, CreatedAt: now, UpdatedAt: now}
	s.jobs[j.ID] = j
...
func (s *Store) EnqueueAuthorizedAttempt(jobID, idempotencyKey, nonce string) (*Attempt, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if nonce == "" {
...
func (s *Store) CompleteAttempt(id, leaseOwner string, status AttemptStatus, errorCode string) (*Attempt, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Store) persist() error {
	...
	b, err := json.MarshalIndent(snapshot{Jobs: s.jobs, Attempts: s.attempts, ConsumedNonces: s.consumedNonces}, "", "  ")
	if err != nil {
		return err
	}
