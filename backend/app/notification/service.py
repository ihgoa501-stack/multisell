...
# 默认预警规则定义
DEFAULT_ALERT_RULES = [
    {"name": "库存低于安全库存", "alert_type": "inventory_low_stock",
     "config": {}, "description": "库存数量 ≤ 安全库存且安全库存 > 0"},
    {"name": "库存为零", "alert_type": "inventory_out_of_stock",
     "config": {}, "description": "库存数量 ≤ 0"},
    {"name": "结算待对账", "alert_type": "settlement_pending",
     "config": {}, "description": "有待对账的结算单超过24小时"},
    {"name": "结算对账差异", "alert_type": "settlement_discrepancy",
     "config": {}, "description": "结算明细对账状态为差异"},
    {"name": "发布失败", "alert_type": "listing_failed",
     "config": {}, "description": "商品发布到平台失败"},
    {"name": "待付款订单超时", "alert_type": "order_pending",
     "config": {"timeout_hours": 24}, "description": "待付款订单超过24小时"},

    {"name": "系统通知", "alert_type": "system",
     "config": {}, "description": "系统级通知"},
]
...
class NotificationService:
    ...
    async def _check_out_of_stock(db: AsyncSession) -> int:
        ...
        for inv in result.scalars().all():
            ...
            if not exists.scalar_one_or_none():
                notif = Notification(
                    user_id=1, alert_type="inventory_out_of_stock",
                    title=f"SKU {inv.sku_id} 已缺货",
                    content="库存数量为 0，请及时补货",
                    link_url=f"/inventory/{inv.sku_id}",
                    severity="error", source_id=f"sku={inv.sku_id}",
                )
                db.add(notif)
                count += 1
        if count:
    ...
    async def list_notifications(
        ...
        stmt = stmt.order_by(desc(Notification.created_at)).offset(offset).limit(page_size)
        result = await db.execute(stmt)
    ...
    @staticmethod
    async def mark_read(db: AsyncSession, notification_id: int, user_id: int = 1) -> bool:
        """标记单条已读"""
    ...
    @staticmethod
    async def delete_notification(db: AsyncSession, notification_id: int, user_id: int = 1) -> bool:
        """删除通知"""
        notif = await db.get(Notification, notification_id)
        if not notif or notif.user_id != user_id:
            return False
    ...
    @staticmethod
    async def update_rule(db: AsyncSession, rule_id: int, data: dict) -> Optional[AlertRule]:
