...
async def _create_exception(async_client) -> int:
    ...
    await async_client.post(
        "/api/shipping/bills/import", files={"file": ("test.csv", content, "text/csv")}
    )
    ...
    async def test_proposal_links_to_exception(self, async_client):
        ...
        resp = await async_client.post(
            ...
            json={
                ...
                "action_type": "resolve_bill",
                "title": "linked",
                "description": "linked to exception",
            },
        )
        data = resp.json()["data"]
...
class TestAgentActionApprove:
    ...
    async def test_approved_then_execute_succeeds(self, async_client):
        ...
        exec_resp = await async_client.post(
            f"/api/agent-actions/{action_id}/mark-executed",
            json={"after_snapshot": {"status": "resolved"}},
        )
    ...
    async def test_rejected_cannot_execute(self, async_client):
        ...
        action_id = cr.json()["data"]["id"]
        await async_client.post(
            f"/api/agent-actions/{action_id}/reject", json={"rejection_reason": "no"}
        )
        exec_resp = await async_client.post(
            f"/api/agent-actions/{action_id}/mark-executed", json={"after_snapshot": {}}
        )
...
class TestAgentActionAudit:
    ...
    async def test_propose_writes_audit(self, async_client):
        ...
        async with async_session_factory() as session:
            stmt = select(OperationLog).where(
                OperationLog.module == "agent_action", OperationLog.action == "propose"
            )
    ...
    async def test_approve_writes_audit(self, async_client):
        ...
        async with async_session_factory() as session:
            stmt = select(OperationLog).where(
                OperationLog.module == "agent_action", OperationLog.action == "approve"
            )
            result = await session.execute(stmt)
            assert len(result.scalars().all()) >= 1
    ...
    async def test_reject_writes_audit(self, async_client):
        ...
        aid = cr.json()["data"]["id"]
        await async_client.post(
            f"/api/agent-actions/{aid}/reject", json={"rejection_reason": "no"}
        )
        from app.database import async_session_factory
        from app.models import OperationLog
        from sqlalchemy import select
        ...
        async with async_session_factory() as session:
            stmt = select(OperationLog).where(
                OperationLog.module == "agent_action", OperationLog.action == "reject"
            )
            result = await session.execute(stmt)
            assert len(result.scalars().all()) >= 1
    ...
    async def test_execute_writes_audit(self, async_client):
        ...
        await async_client.post(f"/api/agent-actions/{aid}/approve")
        await async_client.post(
            f"/api/agent-actions/{aid}/mark-executed", json={"after_snapshot": {}}
        )
        from app.database import async_session_factory
        from app.models import OperationLog
        from sqlalchemy import select
        ...
        async with async_session_factory() as session:
            stmt = select(OperationLog).where(
                OperationLog.module == "agent_action", OperationLog.action == "execute"
            )
            result = await session.execute(stmt)
            assert len(result.scalars().all()) >= 1
