    ...
    async with engine.begin() as conn:
        ...
        for code, name in (
            ...
            r = await conn.execute(
                text(
                    f"INSERT INTO shipping_provider (name, code) SELECT '{name}', '{code}' WHERE NOT EXISTS (SELECT 1 FROM shipping_provider WHERE code='{code}') RETURNING id"
                )
            )
            row = r.fetchone()
            if row:
                providers[code] = row[0]
        for row in (
            await conn.execute(text("SELECT id, code FROM shipping_provider"))
        ).fetchall():
            if row[1] not in providers:
                providers[row[1]] = row[0]
        ...
        for ch, cc, rtype, fkg, fpr, akg, apr in [
            ...
            if not cid:
                continue
            # 创建区域
            zr = await conn.execute(
                text(
                    f"INSERT INTO shipping_zone (channel_id, country_code) SELECT {cid}, '{cc}' WHERE NOT EXISTS (SELECT 1 FROM shipping_zone WHERE channel_id={cid} AND country_code='{cc}') RETURNING id"
                )
            )
            zrow = zr.fetchone()
            zid = (
                ...
                if zrow
                    ...
                    await conn.execute(
                        text(
                            f"SELECT id FROM shipping_zone WHERE channel_id={cid} AND country_code='{cc}'"
                        )
                    )
                ).fetchone()[0]
            )
        ...
        for i, (t, d, s, m) in enumerate(
            ...
            await conn.execute(
                text(
                    f"INSERT INTO exception_item (title,description,severity,source_module,status,created_at) SELECT '{t}','{d}','{s}','{m}','open','{ts}' WHERE NOT EXISTS (SELECT 1 FROM exception_item WHERE title='{t}')"
                )
            )
        ...
        for i, (t, c, a, s) in enumerate(
            ...
            await conn.execute(
                text(
                    f"INSERT INTO notification (user_id,title,content,alert_type,severity,is_read,source_id,created_at) SELECT 1,'{t}','{c}','{a}','{s}',0,'demo-{i}','{ts}' WHERE NOT EXISTS (SELECT 1 FROM notification WHERE title='{t}')"
                )
            )
        ...
        for name, code in [
            ...
            await conn.execute(
                text(
                    f"INSERT INTO warehouse (name, code) SELECT '{name}','{code}' WHERE NOT EXISTS (SELECT 1 FROM warehouse WHERE code='{code}')"
                )
            )
        ...
        for (sid,) in skus:
            ...
            await conn.execute(
                text(
                    f"INSERT INTO inventory_log (sku_id, change_type, change_qty, after_qty, remark) VALUES ({sid}, 'in', {qty}, {qty}, '演示初始化')"
                )
            )
        ...
        products = (
            await conn.execute(
                text(
                    "SELECT p.id, p.name, COALESCE(s.price, 100) FROM product p LEFT JOIN sku s ON s.product_id=p.id AND s.id=(SELECT MIN(id) FROM sku WHERE product_id=p.id) LIMIT 5"
                )
            )
        ).fetchall()
        for oid, name, addr, status, days, items in [
            ...
            if (
                await conn.execute(
                    text(f"SELECT 1 FROM sales_order WHERE order_no='{oid}'")
                )
            ).fetchone():
                continue
            ...
            r = await conn.execute(
                text(
                    f"INSERT INTO sales_order (order_no,recipient_name,shipping_address,status,total_amount,shipping_fee,pay_amount,created_at) VALUES ('{oid}','{name}','{addr}','{status}',0,0,0,'{ts}') RETURNING id"
                )
            )
            dbid = r.fetchone()[0]
            total = 0.0
            for pi, qty in items:
                ...
                uprice = (
                    round(float(price or 100) * 1.15, 2)
                    if price
                    else round(random.uniform(50, 300), 2)
                )
                sub = round(uprice * qty, 2)
                await conn.execute(
                    text(
                        f"INSERT INTO sales_order_item (order_id,sku_id,product_id,product_name,unit_price,quantity,subtotal) VALUES ({dbid},1,{pid},'{pname}',{uprice},{qty},{sub})"
                    )
                )
                total += sub
            ...
            await conn.execute(
                text(
                    f"UPDATE sales_order SET total_amount={round(total, 2)},shipping_fee={sfee},pay_amount={round(total * 1.1, 2)} WHERE id={dbid}"
                )
            )
            await conn.execute(
                text(
                    f"INSERT INTO finance_ledger_entry (entry_type,amount,currency,order_id,cost_layer,description,created_at) VALUES ('revenue',{round(total, 2)},'CNY',{dbid},'product','订单{oid}收入','{ts}')"
                )
            )
